		...
		<div class="content-header row">
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-8">
						<h2 class="content-header-title float-left mb-0">SME Visibility</h2>
					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="<?php echo e(route('admin.faculty-sme.sme-visibility')); ?>" method="get" name="filtersubmit">
									<div class="row">
										<div class="col-lg-3 form-group">
											...
											<select name="category" class="form-control select_category_name">
												...
												?>
												<option value="<?php echo e($val->title); ?>" data-id="<?php echo e($val->id); ?>" <?php echo e(request('category') == $val->title ? 'selected' : ''); ?>>
										...
										<div class="col-lg-3 form-group">
											...
											<select name="mode" class="form-control">
												<option value="">Select</option>
												<option value="Manual" <?php if('Manual' == app('request')->input('mode')): ?> selected="selected" <?php endif; ?>>Manual</option>
												<option value="PrashnKosh" <?php if('PrashnKosh' == app('request')->input('mode')): ?> selected="selected" <?php endif; ?>>PrashnKosh</option>
											</select>
										</div>
										<div class="col-lg-3 form-group">
											...
											<select name="level[]" class="select-multiple form-control" multiple>
												<option value="">Select</option>
												<option value="Easy" <?php if(is_array(app('request')->input('level')) && in_array('Easy', app('request')->input('level'))): ?> selected <?php endif; ?>>Easy</option>
												<option value="Medium" <?php if(is_array(app('request')->input('level')) && in_array('Medium', app('request')->input('level'))): ?> selected <?php endif; ?>>Medium</option>
												<option value="Hard" <?php if(is_array(app('request')->input('level')) && in_array('Hard', app('request')->input('level'))): ?> selected <?php endif; ?>>Hard</option>
											</select>
										</div>
										<div class="col-lg-3 form-group">
											...
											<select name="requirement_for" class="form-control">
												<option value="">Select</option>
												<option value="YouTube" <?php if('YouTube' == app('request')->input('requirement_for')): ?> selected <?php endif; ?>>YouTube</option>
												<option value="Offline Batch" <?php if('Offline Batch' == app('request')->input('requirement_for')): ?> selected <?php endif; ?>>Offline Batch</option>
												<option value="Online Batch" <?php if('Online Batch' == app('request')->input('requirement_for')): ?> selected <?php endif; ?>>Online Batch</option>
											</select>
										</div>
										<div class="col-lg-3 form-group">
											...
											<select name="language" class="form-control">
												<option value="">Select</option>
												<option value="English" <?php if('English' == app('request')->input('language')): ?> selected <?php endif; ?>>English</option>
												<option value="Hindi" <?php if('Hindi' == app('request')->input('language')): ?> selected <?php endif; ?>>Hindi</option>
												<option value="Bilingual" <?php if('Bilingual' == app('request')->input('language')): ?> selected <?php endif; ?>>Bilingual</option>
											</select>
										</div>
										<div class="col-12 col-sm-6 col-lg-3">
											<label for="users-list-role">Employee</label>
											<fieldset class="form-group">
												<select class="form-control employee select-multiple" name="employee">
													...
													<?php $__currentLoopData = $employee; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $em): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
														<option value="<?php echo e($em->id); ?>" <?php echo e(request('employee') == $em->id ? 'selected' : ''); ?>>
															<?php echo e($em->name); ?>
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">
												<select class="form-control faculty select-multiple" name="faculty">
													<option value="">Select Status</option>
													<?php $__currentLoopData = $faculty; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
														<option value="<?php echo e($fa->id); ?>" <?php echo e(request('faculty') == $fa->id ? 'selected' : ''); ?>>
															<?php echo e($fa->name); ?>
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="<?php echo e(route('admin.faculty-sme.sme-visibility')); ?>" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
				...
				<div class="table-responsive">
					<table class="table data-list-view" id="">
						...
						<tbody>
							...
							<tr>
								<td><?php echo e($i++); ?></td>
								<td><?php echo e($re->main_rid); ?></td>
								<td><?php echo e($re->faculty_name); ?></td>
								<td><?php echo e($re->sme_name); ?></td>
								<td><?php echo e($re->category); ?></td>
								<td>
									<?php 
										if(!empty($re->exam)){
											$exam = json_decode($re->exam);										
								...
								<td>
									<?php 
										if(!empty($re->subject)){
											$subject = json_decode($re->subject);										
								...
								<td>
									<?php 
										if(!empty($re->topic)){
											$topic = json_decode($re->topic);										
											foreach($topic as $to){ echo $to->name.',';	}
										}
									?>
								</td>
								<td><?php echo e($re->language); ?></td>
								<td><?php echo e($re->mode); ?></td>
								<td><?php echo e($re->level); ?></td>
								<td><?php echo e($re->requirement_for); ?></td>
								<td><?php echo e($re->no_question); ?></td>
								<td>
									<?php if($re->restatus == 0): ?>
										<span class="badge bg-warning">Pending</span>
									<?php elseif($re->restatus == 1): ?>
										<span class="badge bg-success">Completed</span>
									<?php elseif($re->restatus == 2): ?>
										<span class="badge bg-danger">Rejected</span>
									<?php elseif($re->restatus == 3): ?>
...
$(document).ready(function() {
	...
	$(".get_edit_data").on("click", function() {  
		...
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.faculty-sme.reuse-request')); ?>',
			data : {'_token' : '<?php echo e(csrf_token()); ?>', 'upload_id': upload_id,'request_id':request_id},
			dataType : 'json',
			success : function (data){
				if(data.status == true){
					alert(data.message);
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/admin/faculty_sme/sme_visibility.blade.php ENDPATH**/ ?>