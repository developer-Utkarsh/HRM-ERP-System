			...
			<div class="card">
				<div class="card-content collapse show">
					<div class="card-body">
						...
						<div class="row">
							<div class="col-md-12">
							<form action="<?php echo e(route('admin.timetable.index')); ?>" method="get">
								<div class="row">
									<div class="col-md-3">
									...
									<div class="col-md-3">
										...
										<a href="<?php echo e(route('admin.timetable.index')); ?>" class="btn btn-outline-warning">Reset</a>
						...
						<div class="row">
							<div class="col-md-12">
							<?php if(!empty(app('request')->input('tt_date'))): ?>
							<form action="<?php echo e(route('admin.copy-timetable')); ?>" method="post" id="copy_form_tt">
								<?php echo csrf_field(); ?>
								<div class="row">
									<div class="col-md-4">
										<label for="users-list-role">Date</label>
						...
						<div id="accordion">
							<?php if(count($branch) > 0): ?>
								...
								<?php $__currentLoopData = $branch; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $branchval): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
									<div class="card">
										<div class="card-header" id="heading<?php echo e($i); ?>" style="padding: 0.5rem 0.5rem 0;">
											<h5 class="mb-0">
										...
										<div id="collapse<?php echo e($i); ?>" class="collapse" aria-labelledby="heading<?php echo e($i); ?>" data-parent="#accordion">
											<div class="card-body main_div"> 
												<?php if((!empty($selecteddDate) && $selecteddDate >= date('Y-m-d')) || Auth::user()->role_id == 29): ?>
...
function getBranchTimetable(branch_id,selecteddDate){
    if($('.branch_head'+branch_id).is(':empty')){
	    ...
	    $.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.branch-wise-timetable')); ?>',
...
function confirmPublish(location){
	...
	if(copy_date_val == ''){
		...
		if(confirm('Are you sure to want publish '+location+' Timetable !')){
			$.ajax({
				type : 'POST',
				url : '<?php echo e(route('admin.publish-timetable')); ?>',
...
function confirmUnPublish(location){
	...
	if(copy_date_val == ''){
		...
		if(confirm('Are you sure to want un-publish '+location+' Timetable !')){
			$.ajax({
				type : 'POST',
				url : '<?php echo e(route('admin.unpublish-timetable')); ?>',
				data : {'_token' : '<?php echo e(csrf_token()); ?>', 'copy_date': copy_date_val, 'location': location},
				dataType : 'json',
				success : function (data){
					if(data.status == false){
...
function showDiv(e, branch_id){
	...
	if (branch_id) {
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.get-studio-by-branch')); ?>',
...
function getCourse(e) { 
	...
	if (batch_id) {
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.get-course')); ?>',
...
function getSubject(e) { 
	...
	if (batch_id) {
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.get-class-batch-subject')); ?>',
			data : {'_token' : '<?php echo e(csrf_token()); ?>', 'batch_id': batch_id},
			dataType : 'html',
			success : function (data){ 
...
function getSubjectByFaculty(e){
  var date_val = '<?php echo $selecteddDate; ?>'; 
	...
	if (faculty_id) {
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.get-class-batch-subject-by-faculty')); ?>',
...
function deleteTimetable(e, id){
	if (id) {
		$.ajax({
			...
			type : 'POST',
			url : '<?php echo e(route('admin.delete-timetable')); ?>',
		...
		submitHandler: function(form) {
				...
				// e.preventDefault();
					...
					$.ajax({
						...
						type: "POST",
						url : '<?php echo e(route('admin.timetable.store')); ?>',
						data : dataForm,
						processData : false, 
						contentType : false,
						dataType : 'json',
		...
		submitHandler: function(forms) { 
				...
				// e.preventDefault();
					...
					$.ajax({
						...
						type: "POST",
						url : '<?php echo e(route('admin.edit-timetable')); ?>',
						data : dataForm,
						processData : false, 
						contentType : false,
						dataType : 'json',
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/admin/timetable/index.blade.php ENDPATH**/ ?>