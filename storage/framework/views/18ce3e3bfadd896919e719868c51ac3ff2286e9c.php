			...
			<section id="data-list-view" class="data-list-view-header">
							...
							<div class="users-list-filter">
								<form action="<?php echo e(route('admin.faculty-reports')); ?>" method="get" name="filtersubmit">
										...
										<div class="col-12 col-md-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple2 batch_id" name="batch_id">
													...
													<option value="<?php echo e($value->id); ?>" <?php if($value->id == app('request')->input('batch_id')): ?> selected="selected" <?php endif; ?>>
														<?php echo e($value->name); ?>-<?php echo e($value->erp_course_id); ?></option>
													<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
													<?php endif; ?>
												</select>												
											</fieldset>
										...
										<div class="col-12 col-md-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple3 select_assistant_id" name="assistant_id">
													<option value="">Select Any</option>
													<?php if(!empty($get_assistant) && count($get_assistant) > 0): ?>
									...
									<fieldset class="form-group" style="float:right;">		
										...
										<a href="<?php echo e(route('admin.faculty-reports')); ?>" class="btn btn-warning">Reset</a>
										<?php if( Auth::user()->role_id != 3){ ?>
				...
				?>
					<table class="table data-list-view" style=''>
						...
						<head>
							<tr style="">
								<th colspan="12"><b>Faculty Name : <?php echo isset($get_faculty_value->faculty_name)?$get_faculty_value->faculty_name:''; ?></b> 
									&nbsp;&nbsp;&nbsp;&nbsp; 
									<?php
						...
						<body>
							...
							?>
								<tr>
									...
									<?php 
									$get_batches_name = "";
									$get_batches = DB::table('timetables')->select("batch.name as b_name","batch.erp_course_id")->leftJoin('batch','batch.id','=','timetables.batch_id')->where('timetables.is_deleted', '0')->where('timetables.time_table_parent_id', $get_faculty_timetable_value->id)->get();
									if(count($get_batches) > 0){
										foreach($get_batches as $vallll){
											$get_batches_name .= ', '.$vallll->b_name.'-'.$vallll->erp_course_id;
										}
									}
									echo isset($get_faculty_timetable_value->batch_name) ?  $get_faculty_timetable_value->batch_name.'-'.$get_faculty_timetable_value->erp_course_id : '';
									...
									<td class="spd_time<?php echo e($key2); ?><?php echo e($key); ?>"><?=$duration?></td>
									<td class=""><?=$get_faculty_timetable_value->topic_name?></td>
									<td data-id="<?php echo e($get_faculty_timetable_value->id); ?>">
										<?php
										if(strtotime(date('Y-m-d')) <= strtotime($get_faculty_timetable_value->cdate.'+1 day') || Auth::user()->user_details->degination == 'PRODUCTION INCHARGE' || Auth::user()->user_details->degination == 'CENTER HEAD' || Auth::user()->id == 5760){ ?>
											<a href="javascript:void(0);" data-toggle="modal" data-id="<?php echo e($get_faculty_timetable_value->id); ?>" data-spent-id="<?php echo e($key2); ?><?php echo e($key); ?>" class="btn btn-sm btn-outline-primary get_start_class_data"><span class="action-edit"><i class="feather icon-edit"></i></span></a>
										<?php } ?>
									</td>
								</tr>
							<?php
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<form method="post" id="submit_start_class_form">
						...
						<div class="row pt-2">
							...
							<div class="col-md-12 col-12">
								<div class="form-label-group">
									...
									<input type="hidden" class="form-control chapter_id" placeholder="Sub Topic Name" name="chapter_id" autocomplete="off">
				...
				<hr>
				<div class="row col-12  chapter_topic_list p-2 mx-0" style="display:inherit">
...
<script type="text/javascript">
	...
	$(document).on("change",".chapter_data", function () {
		...
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.get-topic')); ?>',
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/admin/faculty_reports/index.blade.php ENDPATH**/ ?>