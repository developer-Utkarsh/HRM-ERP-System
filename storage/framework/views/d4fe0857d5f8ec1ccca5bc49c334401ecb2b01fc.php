    ...
    <body id="printData">
        ...
        <?php if (!empty($get_studios)) {
			...
			<table class="table2" style="border:1px solid #dee2e6;width: 100%;" >
				...
				<tbody>
				<tr style="">
					<th colspan="7" style="background-color:#1674ba;font-size: 24px;border:0;color:#FFF;"><b style="font-weight: 700;">Branch Name : </b> <span style="font-weight: 100;"><?php echo $branchArray->name; ?> <?php if($branchArray->nickname!=""){ ?>(<?=$branchArray->nickname;?>)<?php } ?> </span>
					<?php
		...
		?>
            <table class="" cellpadding="1" style="width: 100%;font-size: 15px;">
                ...
                <tbody>
					...
					foreach($value->timetable as $key => $timetable){ 
						//echo "<pre>";print_r($timetable);die();
						...
						$fullArray[$key]['batch_name'].="(".(date('M-Y',strtotime($timetable->batch->start_date??''))).")";

						$fullArray[$key]['category_head'] = isset($timetable->batch->categoryhead->name) ? '- Cat. Head: '.$timetable->batch->categoryhead->name : '';
						...
						$tt_id = $timetable->id;
						/*$get_batches = DB::select("SELECT CONCAT(timetables.batch_id,',',GROUP_CONCAT(t2.batch_id)) as batches_id,GROUP_CONCAT(batch.nickname) as b_name FROM `timetables` LEFT JOIN timetables as t2 ON timetables.id = t2.time_table_parent_id LEFT JOIN batch ON  batch.id = t2.batch_id where timetables.time_table_parent_id = 0 and timetables.id = $tt_id and timetables.is_deleted='0' and t2.is_deleted='0'");*/

						$get_batches = DB::select("
						    SELECT 
						        CONCAT(timetables.batch_id, ',', GROUP_CONCAT(t2.batch_id)) AS batches_id,
						        GROUP_CONCAT(
						            CONCAT(
						                batch.nickname, ' (', DATE_FORMAT(batch.start_date, '%b-%Y'), ')'
						            )
						        ) AS b_name
						    FROM timetables
						    LEFT JOIN timetables AS t2 
						        ON timetables.id = t2.time_table_parent_id
						    LEFT JOIN batch 
						        ON batch.id = t2.batch_id
						    WHERE timetables.time_table_parent_id = 0
						      AND timetables.id = $tt_id
						      AND timetables.is_deleted = '0'
						      AND t2.is_deleted = '0'
						");
						
						if(!empty($get_batches[0]->batches_id)){
							$batches_id = $get_batches[0]->batches_id;
							if(!empty($batches_id)){
								$batches_id = explode(',', $batches_id);
								asort($batches_id);
								$batches_id = implode(",",$batches_id);
							}
						}
						else{
							$batches_id = isset($timetable->batch->id) ?  $timetable->batch->id : '';
							if(!empty($batches_id)){
								$batches_id = explode(',', $batches_id);
								asort($batches_id);
								$batches_id = implode(",",$batches_id);
						...
						$fullArray[$key]['all_batches_id'] = $batches_id;
						$fullArray[$key]['all_batches_name'] = $get_batches[0]->b_name;
					...
					<td style="vertical-align: top;width:50%;">
							<table class="table1" cellpadding="1" style="width: 100%;font-size: 14px;border: solid 1px #dee2e6;">
								...
								<tr >
									...
									<?php
									if($logged_id==1096 || $logged_id==5603 ){
									?>
								...
								if(count($fullArray) > 0){
									foreach($fullArray as $key2=>$detail){
										$all_batches_id = $detail['all_batches_id'];
										$checkTime = strtotime($detail['to_time']);
										$checkTime = date('H.i',$checkTime);
										if((float)$detail['to_time'] <= 14){
											if("$batch_id" != "$all_batches_id"){
												$get_batches_name = "";
												...
												if(!empty($detail['all_batches_name'])){
													$get_batches_name = ', '.$detail['all_batches_name'];
												}
											?>
											<tr style="">
												...
												<?=$detail['category_head'];?>
												</th>
											</tr>
											<?php
											}
											$batch_id = $detail['all_batches_id'];
										}
										...
										if("$batch_id" == "$all_batches_id"){
											...
											<tr style="<?=$bgClr;?>">
												...
												<?php 
												if($logged_id==1096  || $logged_id==5603 ){
													echo $schedule_duration;
												}
												else{
													echo $detail['online_class_type'].' - '.$detail['tid']; 
												}
												?>
												</td>
			...
			<?php }
                ...
                <tbody>
					<tr style="" >
                        <th colspan="7" data-a="2" style="background-color:#fff;">
...
</script><?php /**PATH /var/www/html/laravel/resources/views/studiomanager/studio_reports/pdf_html.blade.php ENDPATH**/ ?>