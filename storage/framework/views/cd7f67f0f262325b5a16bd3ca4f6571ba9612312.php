		...
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Branch Wise Free Assistant Report</h2>
					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="<?php echo e(route('studiomanager.free-assistant-reports')); ?>" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<?php $assistant = \App\User::where('role_id', '3')->orderBy('id','desc')->get(); ?>
											<fieldset class="form-group">												
												<select class="form-control select-multiple2 assistant_id" name="assistant_id">
													<option value="">Select Any</option>
													<?php if(count($assistant) > 0): ?>
													<?php $__currentLoopData = $assistant; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
									...
									<a href="<?php echo e(route('studiomanager.free-assistant-reports')); ?>" class="btn btn-warning">Reset</a>
									<a href="javascript:void(0)" id="download_free_assistant_excel" class="btn btn-primary">Export in Excel</a>
									</fieldset>
								</form>
							</div>
						</div>
				...
				if (count($get_data) > 0) { 
					foreach ($get_data as $AssistantArray) { //echo '<pre>'; print_r($FacultyArray); die;
						...
						?>
							...
							<!-- <td></td> -->
							<td><?php 
							if(!empty($AssistantArray->name)){
								echo $AssistantArray->name; 
							} ?>
							</td>
							<td><?php 
							if(!empty($AssistantArray->email)){
								echo $AssistantArray->email; 
							} ?>
							</td>
							<td><?php 
							if(!empty($AssistantArray->mobile)){
								echo $AssistantArray->mobile; 
							} ?>
							</td>
...
<script type="text/javascript">
	...
	$("body").on("click", "#download_free_assistant_excel", function (e) {
		var data = {};
			data.branch_id = $('.branch_id').val(),
			data.assistant_id = $('.assistant_id').val(),
			data.fdate = $('.fdate').val(),
			data.tdate = $('.tdate').val(),
		window.location.href = "<?php echo URL::to('/studiomanager/'); ?>/free-assistant-report-excel?" + Object.keys(data).map(function (k) {
...
<?php echo $__env->make('layouts.studiomanager', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/studiomanager/free_assistant_reports/index.blade.php ENDPATH**/ ?>