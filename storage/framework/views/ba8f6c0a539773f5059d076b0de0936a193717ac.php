		...
		<div class="content-header row">
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-8">
						<h2 class="content-header-title float-left mb-0">Requisition List</h2>
					...
					<div class="col-4 text-right">
						 <button type="button" class="btn btn-outline-info waves-effect waves-light" data-toggle="modal" data-target="#exampleModal">Copy Requisition</button>
					</div>
					<?php } ?>
				</div>
			</div>
		...
		<div class="content-body">			
			<section id="data-list-view" class="data-list-view-header">	
									...
									<div class="row">
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">
												<input type="text" class="form-control rnumber" name="rnumber" value="<?php echo e(app('request')->input('rnumber')); ?>">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-2">
											<label for="users-list-verified">Po / Wo No. </label>
											<fieldset class="form-group">
												<input type="text" class="form-control pwnumber" name="pwnumber" value="<?php echo e(app('request')->input('pwnumber')); ?>">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-3">
											<label for="users-list-verified">Product Title</label>
											<fieldset class="form-group">
												<input type="text" class="form-control pname" name="pname" value="<?php echo e(app('request')->input('pname')); ?>">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-2">
											<label for="users-list-verified">User Name </label>
											<fieldset class="form-group">
												<input type="text" class="form-control uname" name="uname" value="<?php echo e(app('request')->input('uname')); ?>">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-3">
										...
										<div class="col-12 text-right">
											...
											<a href="<?php echo e(route('admin.request.requisition-request')); ?>" class="btn btn-warning">Reset</a>
										</div>
									</div>
								</form>
							</div>
				...
				<div class="table-responsive">
					<table class="table data-list-view">
						...
						<tbody>			
							<?php if(count($notification) > 0): ?>
								<?php $__currentLoopData = $notification; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
									<?php 
										...
										if( Auth::user()->user_details->degination == 'MANAGER-PURCHASE & STORE'){
											$qstatus7 = DB::table('asset_request')
												...
												->where(function($q) use ($value) {
													$q->where('asset_request_notification.purchase_status', 3)
													  ->where('asset_request_notification.pm_status', 0)
													  ->whereNotIn('asset_request_notification.company', ['na', '-']);
												})
												->where('asset_request.unique_no', $value->unique_no)
												->get();
										}
								...
								<tr style="<?=$bgClass;?>"> 
									...
									<td><?php echo e($value->bname); ?></td>
									<td>
										<b>( REQ-<?php echo e($value->unique_no); ?> - <?php echo e($value->dname); ?> )</b> - <?php echo e($value->message); ?> 
										<a href="javascript:void(0)"  data-id="<?php echo e($value->unique_no); ?>" class="get_edit_data text-primary">View</a>										
									</td>
									<td>
										<?php
											//Center Head
									...
									<td>
											...
											if(Auth::user()->user_details->degination == 'CENTER HEAD' && $value->dname == 'ADMIN'){ 
												if(Auth::user()->id != $value->user_id){
													if(count($qstatus) >= 1 ){
														$actionErr = "";
										?> 
										<a href="<?php echo e(route('admin.request.edit-requisition', $value->unique_no)); ?>">
										...
										<?php 
											...
											if( Auth::user()->role_id ==31){
												if(count($qstatus3) >= 1){ 
													$actionErr = "";
										?>										
										<a href="<?php echo e(route('admin.request.edit-requisition', [ $value->unique_no, 1 ])); ?>">
											<span class="action-edit"><i class="feather icon-edit"></i></span>
										</a>										
										<?php }else{ $actionErr = 'Not Editable'; } }?>
										...
										<?php 
											...
											if(Auth::user()->user_details->degination=='MANAGER-PURCHASE & STORE'){ 	
												...
												$actionErr = "";
										?>										
										<a href="<?php echo e(route('admin.request.edit-requisition', [ $value->unique_no, 4 ])); ?>">
											<span class="action-edit"><i class="feather icon-edit"></i></span>
										</a>										
										<?php }else{ $actionErr = 'Not Editable'; } } ?>
										...
										<?php 
											...
											if(count($qstatus4) >= 1){ 
												$actionErr = "";
										?>										
										<a href="<?php echo e(route('admin.request.edit-requisition', [ $value->unique_no, 2 ])); ?>">
											<span class="action-edit"><i class="feather icon-edit"></i></span>
										</a>										
										<?php }else{ $actionErr = 'Not Editable'; } } ?>
											...
											//Inventory		
											 if( Auth::user()->role_id ==25){
												if(count($qstatus5) >= 1){ 
													$actionErr = "";
										?>		
											<a href="<?php echo e(route('admin.request.edit-requisition', [ $value->unique_no, 3 ])); ?>">
												<span class="action-edit"><i class="feather icon-edit"></i></span>
											</a>
										<?php 
												}else{ $actionErr = 'Not Editable'; } 
					...
					<?php echo $notification->appends($params)->links(); ?>
...
<div class="modal fade" id="reqQuotation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<form action="<?php echo e(route('admin.quotation-add')); ?>" method="post" class="form"  enctype="multipart/form-data">
		<?php echo csrf_field(); ?>
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<form action="<?php echo e(route('admin.copy-mrl')); ?>" method="post">
				<?php echo csrf_field(); ?>
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">Requisition</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript">
	...
	$(".get_edit_data").on("click", function() {  
		...
		$.ajax({
			type : 'POST',
			url : '<?php echo e(route('admin.show-requisition')); ?>',
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/admin/request/requisition.blade.php ENDPATH**/ ?>