            ...
            <table class="" cellpadding="1" style="width: 100%;font-size: 15px;">
                ...
                <tbody>
					...
					foreach($value->timetable as $key => $timetable){ 
						...
						$fullArray[$key]['erp_course_id'] = isset($timetable->batch->erp_course_id) ?  $timetable->batch->erp_course_id : '';
						...
						$tt_id = $timetable->id;
						$get_batches = DB::select("SELECT CONCAT(timetables.batch_id,',',GROUP_CONCAT(t2.batch_id)) as batches_id,GROUP_CONCAT(batch.nickname) as b_name,batch.erp_course_id FROM `timetables` LEFT JOIN timetables as t2 ON timetables.id = t2.time_table_parent_id LEFT JOIN batch ON  batch.id = t2.batch_id where timetables.time_table_parent_id = 0 and timetables.id = $tt_id and timetables.is_deleted='0' and t2.is_deleted='0'");
									...
									foreach($fullArray as $key2=>$detail){
										...
										if((float)$checkTime <= 12){
											if("$batch_id" != "$all_batches_id"){
												$get_batches_name = "";
												//$tt_id = $detail['tid'];
												//$get_batches = DB::select("SELECT CONCAT(timetables.batch_id,',',GROUP_CONCAT(t2.batch_id)) as batches_id,GROUP_CONCAT(batch.name) as b_name FROM `timetables` LEFT JOIN timetables as t2 ON timetables.id = t2.time_table_parent_id LEFT JOIN batch ON  batch.id = t2.batch_id where timetables.time_table_parent_id = 0 and timetables.id = $tt_id group by timetables.id");
												if(!empty($detail['all_batches_name'])){
													$get_batches_name = ', '.$detail['all_batches_name'].' ('.($detail['erp_course_id']??'').')';
												}
											...
											<tr style="">
												...
												<?=$detail['batch_name']?> (<?=$detail['erp_course_id']??''?>)
												<?php
										...
										if("$batch_id" == "$all_batches_id"){
											...
											<tr style="<?=$bgClr;?>" >
												...
												<td style="font-size: 13px;"><?php echo $detail['online_class_type'].'-'.$detail['tid']; ?></td>
...
</script><?php /**PATH /var/www/html/laravel/resources/views/admin/studio_reports/pdf_html.blade.php ENDPATH**/ ?>