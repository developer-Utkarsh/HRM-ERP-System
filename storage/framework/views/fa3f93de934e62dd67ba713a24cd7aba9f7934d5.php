								...
								<form id="crm_search" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="<?php echo e(route('admin.crm-desk.search')); ?>" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
    ...
    <form method="post" action="javascript:void(0)" id="form_submit">
      <?php echo e(csrf_field()); ?>
		  ...
		  <div class="modal-body">
				<div class="row">
					<input type="hidden" name="agent_name" class="form-control agent_name" value="<?php echo e($logged_name); ?>" required>
				  ...
				  <!-- <div class="col-md-12 col-12 leadcategory d-none">
					...
					<div class="condition_hs w-100 mx-0 cat_section">
						<div class="row mx-0">
					...
					<div class="row mx-0 w-100">
						<div class="col-md-4 col-12 div_support" style="display:none">
							<div class="row">
						...
						<div class="col-md-4 col-12 div_course">
							<div class="row">
								<div class="col-md-12 col-12">
									<div class="form-group">
										<label for="first-name-column">Course Type* </label>
										<select class="form-control select-ctype C_Type" name="C_Type">
											<option value="">-- Select --</option>
											<option value="From Studio">From Studio</option>											
											<option value="From Jodhpur Classroom">From Jodhpur Classroom</option>										
											<option value="From Jaipur Classroom">From Jaipur Classroom</option>											
											<option value="From Prayagraj Classroom">From Prayagraj Classroom</option>											
											<option value="Non-Enrolled in any course/ batch student">Non-Enrolled in any course/ batch student</option>											
											<option value="Test / Model Paper Query">Test / Model Paper Query</option>											
											<option value="From Indore Classroom">From Indore Classroom</option>											
										</select>
									</div>
								</div>							
							</div>
						</div>
						<div class="col-md-4 col-12 div_support2 StuSupport" style="display:none">
							<div class="row">
								<div class="col-md-12 col-12">
									<div class="form-group">
										<label for="first-name-column">Student Support & Services</label>
										<select class="form-control select-stusupport StudentSupport" name="StudentSupport">
											<option value=""> -- Select --</option>
											<?php foreach($StudentSupport as $ss){ ?>
												<option value="<?php echo e($ss->title); ?>" data-id="<?php echo e($ss->id); ?>"><?php echo e($ss->title); ?></option>											
											<?php } ?>									
										</select>
									</div>
								</div>							
						...
						<div class="col-md-4 col-12 div_support2 StuCategory" style="display:none">
							<div class="row">
								<div class="col-md-12 col-12">
									<div class="form-group">
										<label for="first-name-column">Student Query Category </label>
										<select class="form-control select-qcategory StudentCategory" name="StudentCategory">
						...
						<div class="col-md-4 col-12 div_support2 user_type" style="display:none">
							<div class="row">
								<div class="col-md-12 col-12">
									<div class="form-group">
										<label for="first-name-column">User Type* </label>
										<select class="form-control select-multiple3 u_Type" name="u_Type">
											<option value="">-- Select --</option>
...
<script src="<?php echo e(asset('/laravel/public/js/cities.js')); ?>"></script>
...
$(document).ready(function() {
	...
	$(".select-ctype, .select-stusupport, .select-qcategory").select2({
		tags: false,
		placeholder: "--Select--",
		allowClear: true
	});
	...
	$("input[name='call_related']").change(function(){
		...
		if(call_related=='Inquiry'){
			...
			$(".div_course select option").prop("selected", false);
			$(".div_support2 select option").prop("selected", false);
			...
			$(".div_course select").prop("required", false);			
		}else{
			...
			$(".div_course select").prop("required", true);
		}
	});
	...
	$("select[name='student_type']").change(function(){
      ...
      dynamicSelectBox.innerHTML = `<option value="" > --Support Type-- </option>
			...
			$('.div_course select').prop('required',false);
			$('.div_support2 select').prop('required',false);
...
$("#crm_search").submit(function(e) {
	...
	$.ajax({
		...
		type: "POST",
		url : '<?php echo e(route('admin.crm-desk.search_result')); ?>',
...
function call_activity(e){
	...
	if(id){
		$.ajax({
			...
			type: "POST",
			url : '<?php echo e(route('admin.crm-desk.call_activity')); ?>',
			data : {'id': id},
			dataType : 'json',			
			success : function(data){
				$("#overlay_loader").css('display','none');
...
function sendToNpf(mobile,data){
	$.ajax({
		...
		type: "POST",
		url : '<?php echo e(route('admin.crm-desk.sendToNpf')); ?>',
...
$("#form_reply_submit").submit(function(e) {
	...
	$.ajax({
		...
		type: "POST",
		url : '<?php echo e(route('admin.crm-desk.activity_reply')); ?>',
		data : dataForm,
		processData : false,  
		contentType : false,
		dataType : 'json',
...
$(document).on("change",".agent_assign",function(){
	...
	if(ticket_id && agent_id){
		...
		$.ajax({
			...
			type: "POST",
			url : '<?php echo e(route('admin.crm-desk.assign_agent')); ?>',
...
$(document).on("change",".select_main_category",function(){
	...
	if(main_category_name){
		...
		$.ajax({
			...
			type: "POST",
			url : '<?php echo e(route('admin.crm-desk.get_course_name')); ?>',
...
$("#form_crm_submit").submit(function(e) {
	...
	$.ajax({
		...
		type: "POST",
		url : '<?php echo e(route('admin.crm-desk.save_course')); ?>',
		data : dataForm,
		processData : false,  
		contentType : false,
		dataType : 'json',
...
$(document).on("change", ".C_Type", function () {
    var c_type = $(this).val();
    $('.StuSupport').hide();
	$('.StuSupport select').prop('required',false);
    if (c_type !== '') {
        $('.StuSupport').show();		
		$('.StuSupport select').prop('required',true);
    }
});
...
$(document).on("change",".StudentSupport",function(){
	...
	$('.user_type').hide();
	$('.user_type select').prop('required',false);
	$('.StuCategory select').prop('required',false);
	if(ctype_id==1){
		$('.user_type').show();
		$('.user_type select').prop('required',true);
		
		ndata="Device Name :- \nNetwork:- \nVersion:- \nIssue :-";
		$("textarea[name='description']").val(ndata);
	}else if(ctype_id==3 || ctype_id==4 || ctype_id==5 || ctype_id==6 || ctype_id==7 || ctype_id==11 || ctype_id==12){
		ndata="Main Book id & Course Name :- \nPackage Book id & Subject:- \nTopic id & Name:- \nPart No, Name & id :- \nIssue";
		$("textarea[name='description']").val(ndata);
	}else{
		ndata="";
		$("textarea[name='description']").val(ndata);
	...
	if(ctype_name){
		...
		$.ajax({
			...
			type: "POST",
			url : '<?php echo e(route('admin.crm-desk.get_course_type')); ?>',
			...
			success : function(data){
				...
				if(data.status == false){
					...
					$('.StuCategory').show();
					$('.StuCategory select').prop('required',true);
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/admin/crmdesk/search.blade.php ENDPATH**/ ?>