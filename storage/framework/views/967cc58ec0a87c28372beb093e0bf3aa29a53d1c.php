...
<div class="app-content content" style="margin: 0px !important;">
	<div class="content-wrapper" style="margin-top: 0px !important;">
		<div class="content-body">
		  <div class="">
			  <div class="">
				  <form action="<?php echo e(route('faculty-reports')); ?>" method="get" name="filtersubmit">
						<div class="row">
							<input type="hidden" name="faculty_id" class="faculty_id_get" name="" value="<?php echo e(app('request')->input('faculty_id')); ?>">
							
							<div class="float-left col-9 pr-0">											
								<label for="users-list-status" style="color:#000 !important;"><b>Select Date</b></label>			
								<fieldset class="form-group">
									<input type="date" name="fdate" placeholder="Date" value="<?php echo e($selectFromDate); ?>" class="py-2 form-control StartDateClass fdate" style="border-radius:0;border:solid 1px #c0c0c0;border-top-left-radius:8px;border-bottom-left-radius:8px;">	
								</fieldset>	
							</div>								
				...
				<div class="mb-4" style="font-size:12px; border:solid 1px #c0c0c0;border-radius:15px;">
					<div class="row mx-0 p-2" style="background:#FCDD10;border-top-left-radius:15px;border-top-right-radius:15px">
						...
						<div class="float-left w-75 pr-2">
							<div>Batch Name</div>
						<div><b><?php echo e($batch_name); ?></b></div>
						</div>
                        
                        <?php if($course_planer_enable==1): ?>
						    <div class="float-left w-25 text-right">
								<button type="button" class="btn btn-transparent p-1 viewStatus viewStatus_<?php echo e($val->id); ?>" style="border:solid 1px #000;font-size:12px;color:#000;padding:8px !important" data-id="<?php echo e($val->id); ?>" data-class_duration="<?php echo e($class_duration); ?>"><b>View Status</b></button>
							</div>
						<?php endif; ?>
					</div>
					...
					<div class="px-2 pb-2">
						<div class="py-1">
							...
							?>
							<div>
								<div class="pt-2"><b>Batch : <?php echo e($id); ?> </b> <?= $nBatchName[$key] ?? $id; ?></div>
								<div>
						...
						<div class="acc-container" style="max-width:100%;">
							<div class="acc">
								...
								<?php if(!empty($topic) && count($topic)): ?>
									<div class="acc-head">							
						...
						<div class="text-right">
							<button type="button" class="btn btn-transparent p-1 text-primary raise_issue" style="border:solid 1px #c0c0c0;font-size:12px;padding:8px !important" data-toggle="modal" data-target="#modalRaiseIssue" data-id="<?php echo e($val->id); ?>">
								Raise Issue <i class="fa fa-arrow-right" aria-hidden="true"></i>
							</button>
							<?php if(!empty($topic) && count($topic)): ?> <?php endif; ?>
						</div> 
					</div>
				</div>
...
<script type="text/javascript">
	...
	$(".viewStatus").on("click",function(){
        ...
        $.ajax({
			type: "GET",
			url : '<?php echo e(route('planerDetail')); ?>/'+tt_id,
			dataType : 'json',
	...
	$(document).on("click",".nextClass",function(){
       ...
       var $button = $(this);
        ...
        $.ajax({
			...
			type: "POST",
			url : '<?php echo e(route('scheduleNextTopic')); ?>',
	...
	$(document).on("click", ".get_topic_class", function () {
		...
		$.ajax({
			...
			type: "POST",
			url : '<?php echo e(route('get_topic_class')); ?>',
...
<?php echo $__env->make('layouts.without_login_admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/laravel/resources/views/admin/faculty_reports/without_login_index.blade.php ENDPATH**/ ?>