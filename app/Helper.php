...
class Helper{
	...
	//Multi Course Part_1
	public function plannerDetails_part_1($tt_id,$batch_id=0,$subject_id=0){
		$planners=[];
		if($tt_id!=0){
			$timetable = DB::table('timetables')
		  ->select('timetables.id','timetables.batch_id',
	...
	public function plannerDetails($tt_id,$batch_id=0,$subject_id=0){
		...
		foreach($timetable as $val){
			...
			if($val->master_planner==0){
				$completed_topic = DB::table('timetable_topic as tt')
					->selectRaw("
						chapter.id,
						chapter.id as chapter_id,
						chapter.name as chapter_name,
						chapter.duration,
						SUM(t.spent_minutes) AS spent_hours,
						SUBSTRING_INDEX(
							GROUP_CONCAT(tt.status ORDER BY tt.timetable_id DESC SEPARATOR ','),
							',', 1
						) AS status,
						DATE_FORMAT(MAX(t.cdate), '%d-%m-%Y') AS cdate,
						MAX(tt.timetable_id) AS timetable_id 
					")
					->leftJoin('chapter', 'chapter.id', '=', 'tt.chapter_id')
					->leftJoin('timetables as t', 't.id', '=', 'tt.timetable_id')
					->where('tt.subject_id', $val->subject_id)
					->where('tt.batch_id', $val->batch_id)
					->groupBy('chapter.id', 'chapter.name', 'chapter.duration')
				...
				$chapters= DB::table('chapter')->selectRaw("chapter.id,chapter.id as chapter_id,chapter.name as chapter_name,chapter.duration,null as status,0 as spent_hours")
				->where("chapter.course_id", $val->course_id)
				->where("chapter.subject_id",$val->subject_id)
				->where("chapter.status",1)
				->where("chapter.is_deleted",'0')
				->whereNOTIN("chapter.id",$topic_ids)
				...
				/*
				$completed_topic = DB::table('timetable_topic as tt')
				->selectRaw("chapter.id,chapter.id as chapter_id,chapter.name as chapter_name,cptr.duration,tt.status,DATE_FORMAT(t.cdate, '%d-%m-%Y') AS cdate,IFNULL(sc_summary.spent_minutes, 0) as spent_hours")
				->leftJoin('timetables as t', 't.id', '=', 'tt.timetable_id')
				->leftJoin('batch', 'batch.id', '=', 't.batch_id')
				->leftJoin('course_planner_topic_relation as cptr', function($join) {
					$join->on('cptr.req_id', '=', 'batch.master_planner')
						 ->on('cptr.topic_id', '=', 'tt.chapter_id');
				})
				->leftJoin('topic_master as chapter', 'chapter.id', '=', 'tt.chapter_id')
				->leftJoin(DB::raw('(
					SELECT 
						timetable_id, 
						ROUND(SUM(TIME_TO_SEC(TIMEDIFF(end_time, start_time))) / 60) as spent_minutes
					FROM start_classes
					GROUP BY timetable_id
				) as sc_summary'), function($join) {
					$join->on('sc_summary.timetable_id', '=', 'tt.timetable_id')
						 ->where('tt.status', '=', 1);
				})
				->where('tt.subject_id', $val->subject_id)
				->where('tt.batch_id', $val->batch_id)
				// ->where('cptr.req_id', $val->master_planner)
				->groupBy('chapter.id', 'chapter.name', 'cptr.duration', 'tt.status', 'sc_summary.spent_minutes', 't.cdate')
				->orderBy('tt.id', 'asc')
				->get();
				...
				$comman_chapters = DB::table('course_planner_topic_relation as cptr')
					->select(
						...
						DB::raw('0 as status'),
						DB::raw('NULL as cdate'),
						DB::raw('0 as spent_hours')
					)
					->join('topic_master as chapter', 'chapter.id', '=', 'cptr.topic_id')
					->whereIn('cptr.topic_id', function ($query) use ($masterPlannerList) {
					...
					->where('cptr.subject_id', $subject_id)
					->whereNotIn('cptr.topic_id', function ($sub) use ($val, $masterPlannerList) {
						$sub->select('tt.chapter_id')
							->from('timetable_topic as tt')
							->join('timetables as t', 't.id', '=', 'tt.timetable_id')
							->where('tt.status', 1)
							->where('tt.subject_id', $val->subject_id)
							->whereIn('tt.batch_id', function ($q) use ($masterPlannerList) {
								$q->select('id')
								  ->from('batch')
								  ->whereIn('master_planner', $masterPlannerList);
							});
					})
					// ->distinct()
				...
				foreach($comman_chapters as $c_c){
					...
					foreach ($completed_topic as $item) {
						if ($item['chapter_id'] == $c_c->id) {
							$c_c->status=$item['status']??0;
							$c_c->cdate  = $item['cdate'] ?? null;
							$c_c->spent_hours  = $item['spent_hours'] ?? 0;
							break;
						}
					}
					$ctopic[]=$c_c;
				...
				->leftJoin('topic_master as chapter', 'chapter.id', '=', 'cptr.topic_id')
				->where('chapter.status', 1)
				->whereNotIn('chapter.id', $topic_ids2)
				->where('cptr.req_id', $val->master_planner)
				->where('cptr.subject_id', $subject_id)
				->orderBy('chapter.id', 'asc')
				->get();
				...
				*/
				...
				->where('tt.subject_id', $val->subject_id)
				->where('tt.batch_id', $val->batch_id)
				->groupBy('chapter.id', 'chapter.name', 'cptr.duration')
				->orderBy('chapter.id', 'desc')
				->get();
				...
				$completed_topic_map = [];
				foreach($completed_topic as $c_t){
					$completed_topic_map[$c_t->id] = $c_t;
				}

				$unique_completed_topics = array_values($completed_topic_map);
				$completed_topic_ids = array_keys($completed_topic_map);
				...
				$filtered_uncompleted_common_chapters = [];
				$comman_chapters_array = json_decode(json_encode($comman_chapters), true);
				foreach ($comman_chapters_array as $c_c) {
					if (!in_array($c_c['id'], $completed_topic_ids)) {
						$filtered_uncompleted_common_chapters[] = $c_c;
					}
				}

				$comman_chapters = array_merge(
					json_decode(json_encode($unique_completed_topics), true),
					$filtered_uncompleted_common_chapters
				);

				$combined_topic_ids = array_column($comman_chapters, 'chapter_id');
				...
				$chapters = DB::table('course_planner_topic_relation as cptr')
					...
					->leftJoin('topic_master as chapter', 'chapter.id', '=', 'cptr.topic_id')
					->where('chapter.status', 1)
					->whereNotIn('chapter.id', $combined_topic_ids)
					->where('cptr.req_id', $val->master_planner)
					->where('cptr.subject_id', $subject_id)
					->orderBy('chapter.id', 'asc')
					->get();
