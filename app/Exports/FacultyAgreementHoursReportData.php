...
class FacultyAgreementHoursReportData implements FromCollection,WithHeadings,ShouldAutoSize,WithEvents
{
	...
	public function __construct($get_faculty,$mt,$yr,$branch_location){ 
				...
				if(!empty($branch_location) ){
					if(count($get_total_time) > 0){
						...
						if(!empty($schedule_total_tt)){
							$export_data[$dataFound][] = $schedule_total_tt;
						} else{
							$export_data[$dataFound][] = '';
						}
						
						if(!empty($total_tt)){
							$export_data[$dataFound][] = $total_tt;
						} else{
							$export_data[$dataFound][] = '';
						}
