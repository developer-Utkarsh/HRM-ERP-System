            ...
            if(isset($emp_id) && !empty($emp_id)){
				...
				if(!empty($user)){
					if($user->status=='1'){
						...
						if(!empty($user_data)){
							$employees_array[] = array('emp_id'=>$emp_id,'name'=>$user_data->name,'role_id'=>$user_data->role_id); //Self	
						}
						...
						if(!empty($request->name)){
							...
							// $check_supervisor->where("name", "LIKE", "%$name%");
						}
						...
						if(count($check_supervisor) > 0){
							...
							foreach($employees as $key=>$value){
								if(!empty($value)){
									$employees_array[] = array('emp_id'=>$value->id,'name'=>$value->name,'role_id'=>$value->role_id);
								}
							}
						}
						
						$date_wise_attendance = Task::groupBy('date');
						if(!empty($request->date_from) && !empty($request->date_to)){
							$date_wise_attendance->where('date', '>=', $request->date_from);
							$date_wise_attendance->where('date', '<=', $request->date_to);
						}
						else{
							$date_wise_attendance->whereMonth('date', '=', date('m'));
						}
						$date_wise_attendance = $date_wise_attendance->orderBy('date', 'desc')->get();
						...
						if(count($date_wise_attendance) > 0){
							foreach($date_wise_attendance as $key=>$valAtt){
								...
								foreach($employees_array as $key2=>$employeeId){
									...
									$task = Task::with(['task_details'=>function($query) use ($emp_id, $user_role_id) {
									...
									$Deleted = "Deleted";
									$task->WhereHas('task_details', function ($q) use ($Deleted,$emp_id,$user_role_id) {
