...
class StaffmovementsystemController extends Controller
{
    public function add_staff_movement_system(Request $request)
    {
    	try { 
			...
			if(isset($emp_id) && !empty($emp_id)){
				...
				if(!empty($user)){
					if($user->status=='1'){
						$inputs = $request->only('emp_id','from_time','to_time','reason');
						
						$inputs['cdate'] = date('Y-m-d');
						if($from_time >= date('H:i') && $to_time > $from_time){
							$save = StaffMovementSystem::create($inputs);
							if($save->save()){
								return $this->returnResponse(200, true, "Staff Movement System save successfully");
							}else{
								return $this->returnResponse(200, false, "Something went wrong.");
							}
						}
						else{
							return $this->returnResponse(200, false, "From Time should be greater than current time and To Time should be greater than From Time.");
						}
					}
					else{
							...
							if(!empty($staff)){
								...
								if(!empty($update_data)){
									if($from_time >= date('H:i') && $to_time > $from_time){
										...
										if($update){
											return $this->returnResponse(200, true, "Staff Movement System save update successfully");
										}
										else{
											return $this->returnResponse(200, false, "Staff Movement System save not update. Please try again.");
										}
									}
									else{
										return $this->returnResponse(200, false, "From Time should be greater than current time and To Time should be greater than From Time.");
	...
	public function get_staff_movement_system(Request $request)
    {
        try{
            ...
            $emp_id  = $request->emp_id;
			$logid   = array();
			$users   = NewTask::getEmployeeByLogID($emp_id); 
			$logid[] = $emp_id;
			if(!empty($users)){
						...
						if(count($report) > 0){
							foreach($report as $key=>$value){
								...
								if(!empty($value)){
									...
									$check_my_emp = User::whereRaw('supervisor_id LIKE  \'%"'.$value->emp_id.'"%\' ')->first();
