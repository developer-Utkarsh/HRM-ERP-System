...
{   
    ...
    public function salary(Request $request)
    {
    	try {
			...
			if(!empty($request->emp_id)){
				$whereCond .= " AND (users.id IN ($request->emp_id))";
			}
			else{
				$whereCond .= ' AND users.role_id != 1 AND users.status = 1';
			}	
			
		    if(!empty($request->emp_id)){
				if(date('Y-mm') != $yr.'-'.$mt){ 
					$attendance = User::query();
					$attendance->select(\DB::raw("users.id as id,users.register_id,users.name as name,users.email as email,users.mobile as mobile,users.inactive_date,users.status as user_status, branches.id as branch_id,branches.name as branch_name,branches.branch_location as branch_location, departments.name as departments_name,userdetails.degination as designation_name,userdetails.joining_date,users.total_time,users.is_extra_working_salary,users.reason_date,attendance_additional_days.days as additional_days"));
					...
					$attendance->leftJoin('attendance_additional_days', function($join) use ($yr,$mt) {
						$join->on('attendance_additional_days.user_id', '=', 'users.id')->where('monthyear','=', "$yr-$mt");
					});
					$attendance->whereRaw($whereCond);
					...
					if(count($array1) > 0){
						foreach($array1 as $val){
							...
							$total_present_half_count = $total_present_half;
							if($total_present_half > 0){
								$total_present_half = ($total_present_half/2);
							}
							
							// $actual_paid = $total_present + $total_present_half + $total_week_off + $holiday + $total_pl + $total_cl + $total_sl + $total_holiday_working;
							...
							$user_array['full_day_present'] = $full_day_present + $total_present_half;
							$user_array['total_present_half'] = 0;
							...
							$user_array['approved_pl'] = $total_pl;
							$user_array['approved_cl'] = $total_cl;
							$user_array['approved_sl'] = $total_sl;
							$user_array['approved_co'] = $total_co;
							$user_array['is_comp_off'] = $is_comp_off;
							// $user_array['total_present'] = ($total_present+$total_present_half);
							$user_array['total_present'] = ($actual_paid);
					...
					if(count($comman_result) > 0){
						foreach($comman_result as $key=>$valAtt){
							...
							$responseArray[$key]['full_day_present'] = $valAtt['full_day_present'];
							...
							$responseArray[$key]['approved_pl'] = $valAtt['approved_pl'];
							$responseArray[$key]['approved_cl'] = $valAtt['approved_cl'];
							$responseArray[$key]['approved_sl'] = $valAtt['approved_sl'];
							$responseArray[$key]['approved_co'] = $valAtt['approved_co'];
							$responseArray[$key]['is_comp_off'] = $valAtt['is_comp_off'];
					...
					foreach($responseArray as $key=>$value){ 	
						$responseArr['half_day_present'] = $value['total_present_half'];
						$responseArr['full_day_present'] = $value['full_day_present'];
						$responseArr['absent']           = $value['total_absent'];		
						$responseArr['holiday_working']  = $value['total_holiday_working'];					
						$responseArr['total_week_off']   = $value['total_week_off'];
						$responseArr['approved_pl']   = $value['approved_pl'];
						$responseArr['approved_cl']   = $value['approved_cl'];
						$responseArr['approved_sl']   = $value['approved_sl'];
						$responseArr['approved_co']   = $value['approved_co'];
						$responseArr['is_comp_off']   = $value['is_comp_off'];
						$responseArr['total_present']    = $value['total_present'];
						$responseArr['additional_days']    = $value['additional_days'];
					} 
					
					
					$data['salary'] = $responseArr;
					return $this->returnResponse(200, true, "Salary Details", $data);
			    }else{
					$responseArr['half_day_present'] = 0;
					$responseArr['full_day_present'] = 0;
					$responseArr['absent']           = 0;
					$responseArr['holiday_working']  = 0;		
					$responseArr['total_week_off']   = 0;
					$responseArr['approved_pl']   = 0;
					$responseArr['approved_cl']   = 0;
					$responseArr['approved_sl']   = 0;
					$responseArr['approved_co']   = 0;
					$responseArr['is_comp_off']   = 0;
					$responseArr['total_present']    = 0;
					$responseArr['additional_days']    = 0;
	...
	public function salary2(Request $request)
    {
    	try {
			...
			$whereCond = ' 1=1 ';
			if(!empty($request->emp_id)){
				$whereCond .= ' AND id = "'.$request->emp_id.'"';
			}
			else{
				$whereCond .= ' AND role_id != 1 AND status = 1';
			}
			
			
			//$get_emp = User::with('user_details')->where([['role_id', '!=', 1], ['status', '=', 1]])->paginate(50);
			$get_emp = User::with('user_details') ->whereRaw($whereCond)->get();
			//$get_emp = User::with('user_details') ->whereRaw($whereCond)->paginate(50);
			...
			if(count($get_emp) > 0){
				...
				//echo '<pre>'; print_r($first_day_month.$last_day_month);die;
				foreach($get_emp as $key=>$value){ 
					...
					$responseArray[$key]['working_days']   = $count_this_month_attds;
					$responseArray[$key]['monthly_salary'] = $value->user_details->net_salary;
					$responseArray[$key]['salary']         = $per_day_salary * $count_this_month_attds;


                    $responseArray[$key]['half_day_present']="";
                    $responseArray[$key]['full_day_present']="";
                    $responseArray[$key]['absent']="";
                    $responseArray[$key]['holiday_working']="";
                    $responseArray[$key]['total_present']=NULL;
                    $responseArray[$key]['total_week_off']=null;*/

                    $responseArray['half_day_present']="";
                    $responseArray['full_day_present']="";
                    $responseArray['absent']="";
                    $responseArray['holiday_working']="";
                    $responseArray['total_present']=NULL;
                    $responseArray['total_week_off']=null;
				} 
				
				$data['salary'] = $responseArray;
				//$data= $responseArray;
				return $this->returnResponse(200, true, "Salary Details",$data);
			}
			else{
	...
	public function addAttendence(){ die;
		try {
			...
			$get_emp = User::where([['role_id', '!=', 1], ['status', '=', 1], ['id', '>=', 5302], ['id', '<=', 5401]])->get();
			//echo '<pre>'; print_r(count($get_emp));die;
			foreach($get_emp as $get_emp_value){
				$empsql = "";
				foreach($period as $period_val){
					
					$empid = $get_emp_value->id;
					$date = $period_val->format('Y-m-d');
					
					$sin_sql = "INSERT INTO `attendance` (`emp_id`, `date`, `time`, `type`) SELECT '$empid', '$date', '09:00', 'In' FROM DUAL WHERE NOT EXISTS (SELECT * FROM `attendance`  WHERE `emp_id`='$empid' AND `date`='$date' AND `type`='In' LIMIT 1) AND NOT EXISTS (SELECT * FROM `leave` JOIN `leave_details` ON `leave`.id=leave_details.leave_id where `leave`.emp_id = '$empid' and leave_details.date='$date' and leave_details.status='Approved' LIMIT 1); INSERT INTO `attendance` (`emp_id`, `date`, `time`, `type`) SELECT '$empid', '$date', '18:00', 'Out' FROM DUAL WHERE NOT EXISTS (SELECT * FROM `attendance` WHERE `emp_id`='$empid' AND `date`='$date' AND `type`='Out' LIMIT 1) AND NOT EXISTS (SELECT * FROM `leave` JOIN `leave_details` ON `leave`.id=leave_details.leave_id where `leave`.emp_id = '$empid' and leave_details.date='$date' and leave_details.status='Approved' LIMIT 1);";
					//echo '<pre>'; print_r($sin_sql);die;	
					$empsql .= $sin_sql;
					
				}
				$res = DB::unprepared($empsql);
