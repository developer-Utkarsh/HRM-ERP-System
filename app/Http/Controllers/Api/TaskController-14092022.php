...
class TaskController extends Controller
{
    ...
    { 
        try{  
		//echo '<pre>'; print_r($request->task_added_by);die;   
			if(isset($request->task_added_by) && !empty($request->task_added_by)){
				$user = User::where('id', $request->task_added_by)->first();
				...
				if(!empty($user)){
					...
					if($user->status=='1'){
						...
						if(count($task_arr) > 0){
							foreach($task_arr as $tsk_value){

								$parent_id = 0;
								if(!empty($tsk_value['task_id'])){
									$parent_id = $tsk_value['task_id'];
								}
								$saveData = array(
											'task_date' => $tsk_value['task_date'],
											'task_title' => $tsk_value['task_title'],
											'task_description' => $tsk_value['task_description'],
											'task_added_by' => $request->task_added_by,
											'plan_hour' => $tsk_value['plan_hour'],
											'spent_hour' => $tsk_value['spent_hour'],
											'parent_id' => $parent_id,
											'task_type' => $tsk_value['task_type'],
											'task_priority' => $tsk_value['task_priority'],
											'status' => $tsk_value['status'],
											'remark' => $tsk_value['remark']
											);
											
								if(!empty($tsk_value['task_added_to'])){
									$saveData['task_added_to'] = $tsk_value['task_added_to'];
								}
								
								if(!empty($tsk_value['task_file_link'])){
									
									$encoded_string = $tsk_value['task_file_link'];
									$target_dir = public_path(DIRECTORY_SEPARATOR . 'task' . DIRECTORY_SEPARATOR);; // add the specific path to save the file
									...
									try {
										...
										$saveData['task_file_link'] = 'task/'.$fileName;
									} catch (Exception $e) {
						...
						return $this->returnResponse(200, true, "Successfully Task Assign");

					}
					else{
						return $this->returnResponse(200, false, "Task Add By Not Active");
					}

				}
				else{
					return $this->returnResponse(200, false, "Task Add By Not Found");  
				}
			}
			else{
				return $this->returnResponse(200, false, "Task Add By is required");  
			}        
        } catch (\Illuminate\Database\QueryException $ex) {
            return $this->returnResponse(500, false, $ex->getMessage());
        } catch (ModelNotFoundException $ex) {
    ...
    public function new_update_task(Request $request)
    {
        try{
        	$task_added_by = $request->task_added_by; 
            $task_id       = $request->task_id;
        	if(isset($task_added_by) && !empty($task_added_by) && isset($task_id) && !empty($task_id)){
        		$user = User::where('id', $task_added_by)->first();

        		if(!empty($user)){

        			if($user->status=='1'){

        				$task = NewTask::where('id', $task_id)->first();
						if(!empty($task)){
							$data = array();


							if($request->status == "In Progress" || $request->status == "Not Started"){
								$t_date = $task->task_date;
							}
							else{
								$t_date = $request->task_date;
							}

							if(!empty($t_date)){
								$data['task_date'] = $t_date;
							}

							if(!empty($request->task_title)){
								$data['task_title'] = $request->task_title;
							}
							if(!empty($request->task_description)){
								$data['task_description'] = $request->task_description;
							}
							if(!empty($request->task_added_by)){
								$data['task_added_by'] = $request->task_added_by;
							}
							if(!empty($request->task_added_to)){
								$data['task_added_to'] = $request->task_added_to;
							}
							if(!empty($request->plan_hour)){
								$data['plan_hour'] = $request->plan_hour;
							}
							if(!empty($request->spent_hour)){
								$data['spent_hour'] = $request->spent_hour;
							}
							if(!empty($request->task_type)){
								$data['task_type'] = $request->task_type;
							}
							if(!empty($request->task_file_link)){
								$data['task_file_link'] = $request->task_file_link;
							}
							...
							$task_details = NewTask::find($request->task_id);
							$task_details->update($data);
							...
							if(!empty($request->task_id)){
                                $task_history = TaskHistory::create([
			                            ...
			                            'task_added_by' => $request->task_added_by,
			                            'task_added_to' => $request->task_added_to,
			                            'plan_hour' => $request->plan_hour,
			                            'spent_hour' => $request->spent_hour,
			                            'parent_id' => $task->parent_id,
			                            'task_type' => $request->task_type,
			                            'task_file_link' => $request->task_file_link,
                            ...
                            if(!empty($request->status)){
	                            if($request->status == "In Progress" || $request->status == "Not Started"){
	                            	...
	                            	if(!empty($request->task_date) && $request->task_date == date('Y-m-d')){
		                            	$next_day_date = date("Y-m-d", strtotime("+ 1 day"));		
										$day = date('D', strtotime($next_day_date));
										if(strtolower($day)=="sun"){
											$next_day_date = date("Y-m-d", strtotime("+ 2 day"));
										...
										$next_day_date = $request->task_date;
									}


									$task = NewTask::create([
		                            'task_date' => $next_day_date,
		                            'task_title' => $request->task_title,
		                            'task_description' => $request->task_description,
		                            'task_added_by' => $request->task_added_by,
		                            'task_added_to' => $request->task_added_to,
									...
									if(!empty($request->task_added_by) && !empty($request->task_added_to) && $request->task_added_by != $request->task_added_to && $task->task_added_to != $request->task_added_to){

			                        	$task_added_to_detail = User::where('id', $request->task_added_to)->first();
    ...
    { 
        try{
            ...
            if(isset($task_id) && !empty($task_id)){
				$updateData['status'] = "Deleted";
				$delete_response = NewTask::where('id', $task_id)->update($updateData);
				if($delete_response){
					return $this->returnResponse(200, true, "Delete Successfully");
				}
				else{
					return $this->returnResponse(200, false, "Id Not Correct");
				}
