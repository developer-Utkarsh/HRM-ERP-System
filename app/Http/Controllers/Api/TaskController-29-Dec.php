						...
						if(count($date_wise_attendance) > 0){
							foreach($date_wise_attendance as $key=>$valAtt){
								...
								foreach($employees_array as $key2=>$employeeId){
									...
									$task = Task::with(['task_details'=>function($query) use ($emp_id, $role_id) {
												$query->where('assigned_userid', $emp_id);
													if($role_id==29){ //Super Admin
									...
									$Deleted = "Deleted";
									$task->WhereHas('task_details', function ($q) use ($Deleted,$emp_id,$role_id) {
