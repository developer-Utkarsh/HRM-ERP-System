				...
				if(!empty($user)){
					if($user->status=='1'){
						...
						if(!empty($request->date_from) && !empty($request->date_to)){
							...
							$date_wise_attendance->whereMonth('assigned_date', '=', date('m'));
						}
						...
						if(count($date_wise_attendance) > 0){
							foreach($date_wise_attendance as $key=>$valAtt){
								...
								foreach($employees_array as $key2=>$employeeId){
									...
									$task = Task::with(['task_details'=>function($query) use ($emp_id, $user_role_id,$date) {
