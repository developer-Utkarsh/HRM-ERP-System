	   ...
	   // echo '<pre>'; print_r($comman_result);die;
		...
		if(count($comman_result) > 0){
			foreach($comman_result as $key=>$valAtt){ 
				...
				$responseArray[$key]['total_hours'] = "00";
				$responseArray[$key]['reason'] = $valAtt->reason;
			...
			while(($filesop=fgetcsv($file, 10000, ",")) !== FALSE) {
				...
				if($jj!=1){  
					...
					if(!empty($emp_id)){
						...
						if(!empty($user)){
							$msg = "आपके नवम्बर माह मे कुल $attendance दिनों की उपस्थिति दर्ज की गई हैं इसी के आधार पर आपको नवम्बर माह का वेतन मिलेगा इसमें अगर किसी प्रकार की त्रुटि हो तो आप कल सुबह 10 बजे से पहले अपने डिपार्टमेंट हेड को बताए और HRMS app में update करवाए।";
							$user_id = $user->id;
							DB::table('api_notifications')->insertGetId([
								'sender_id' =>  901,
								'receiver_id' =>  json_encode(array("$user_id")),
								'title' =>  "Month NOVEMBER-2021 Attadance Update!!",
								'description' =>  $msg,
								'type' => 'General',
