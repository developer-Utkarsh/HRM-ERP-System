...
class FacultyReportsController extends Controller
{
    ...
    public function index()
    {
		...
		if(!empty(Auth::user()->id)){
			...
			return view('admin.faculty_reports.without_login_index', compact('get_faculty','selectFromDate','selectToDate','drivers'));
		}
   ...
   public function download_pdf() {
		...
		return view('admin.faculty_reports.pdf_html', compact('get_faculty','selectFromDate','studio_arr', 'status'));
	...
	public function editStartClass(Request $request){
		if(!empty($request->tt_id)){
			...
			$s_time = ''; $t_time = '';$t_topic_name = '';$remark = '';$res = '';
			$chapters = "<option value=''>Select Topic </option>";
			$get_timetable = DB::table('timetables')->select('batch_id','course_id','subject_id','chapter_id')->where('id', $timetable_id)->where('is_deleted', '0')->first();
			if(!empty($get_timetable)){
				$course_id = $get_timetable->course_id;
				$subject_id = $get_timetable->subject_id;
				...
				if(!empty($get_chapter)){
					foreach($get_chapter as $chapter_data){
						$chapters .="<option value='$chapter_data->id'>$chapter_data->name </option>";
					}
				}
				...
				$start_class_data = DB::table('start_classes')->where('timetable_id', $timetable_id)->first();
				if(!empty($start_class_data)){
					...
					$timetable_detail = DB::table('timetables')->select('from_time','to_time')->where('id', $timetable_id)->first();
			...
			return response(['status' => true, 'start_time' => $s_time, 'end_time' => $t_time, 'topic_name' => $t_topic_name, 'remark' => $remark, 'res' => $res,'total_spent_time'=>$total_spent_time,'chapters'=>$chapters], 200);
		}
		else{
			 return response(['status' => false, 'message' => 'Something Went Wrong'], 200);
		}
						...
						if($start_time >= strtotime($timetable_detail->from_time)){
							...
							if($total_hours < 4){
								...
								if(!empty($get_prev_data->start_time) && !empty($get_prev_data->end_time) && ($get_prev_data->start_time != $request->start_time || $get_prev_data->end_time != $request->end_time)){
									...
									DB::table('start_classes')->where('timetable_id', $request->timetable_id)->update([
								...
								DB::table('timetables')->where('id', $request->timetable_id)->update(['is_publish' => '1', 'is_cancel' => '0', 'is_deleted' => '0']);
