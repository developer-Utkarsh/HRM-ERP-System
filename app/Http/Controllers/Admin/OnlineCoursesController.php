...
class OnlineCoursesController extends Controller
{
    ...
    public function __construct(){
     $this->courseArr=array(5453,5441,1089,1207,1237,1069,1556,1868,5785,1246,1096,1215,1926,5525,5603,1926,1078,5006,1840,1025,1546,1292,5140,6707,7074,7860,1027,1522,1540,5753,8462,8708,1661,1665);
    }
    ...
    public function index()
    {
		if(in_array(Auth::user()->id, $this->courseArr)){
			...
			if(Auth::user()->id==5525){
				// Nirmal pathak sir user_id 5525
				$login_id = Auth::user()->id;
				$employees = User::where('status', '1')
					->whereRaw('is_deleted = "0" and role_id !=1 and ( id = '.$login_id.' or department_type = '.$department_id.' or supervisor_id LIKE \'%"'.$login_id.'"%\' )')->get(); 
			}
			else if(Auth::user()->id==7074){
				// Kanti sir user_id 5525
				$login_id = Auth::user()->id;
				$employees = User::where('status', '1')
					->whereRaw('is_deleted = "0" and role_id !=1 and ( department_type = 34 )')->get(); 
			}
			else if(Auth::user()->id==1025){
				// Chunider Sir user_id 1025
				$login_id = Auth::user()->id;
				$user_branch= Auth::user()->user_branches[0]->branch_id;

				$employees = User::select('users.*')->join('userbranches','users.id', '=', 'userbranches.user_id')
				->where('userbranches.branch_id',$user_branch)
				->where('users.status', '1')->get(); 
			}else if(Auth::user()->id==1096 || Auth::user()->id==1665){
				// Poonam Sir user_id 1096
				$employees = User::where('status', '1')->get(); 
			}else{
				$employees = User::where('status','1')->where('is_deleted','0')->where('department_type',$department_id)->get();
				//->pluck('name', 'id');
			}
			...
			return view('admin.onlinecourses.index', compact('employees','online_course_result','batchcourses'));
		}else{
		...
		if(in_array(Auth::user()->id, $this->courseArr)){
			...
			if(!empty($selectEmpDetails)){
                ...
                if($request->course_type=='Online Course'){
					...
					return redirect()->route('admin.onlinecourses.index')->with('error', 'Something is wrong');
				}
				...
				for($i=0;$i<count($course_id);$i++){
				    ...
				    if(!empty($c_id)){
					  	if($request->course_type=='Online Course'){
					        $paymentId  = $c_id.'-'.rand(10,99).'-employee-'.$request->emp_id;
					        $api_name="addBooktoUser?userName={$contact_no}&courseId={$c_id}&siteId=1&paymentId=$paymentId&remarks=$remarks&amount=$amount&pay_via=HRM";
		                }else if($request->course_type=='Batch Course'){
				...
				if(empty($add_book_status)){
					return redirect()->route('admin.onlinecourses.index')->with('success', 'Successfully added');
				}else{
					return redirect()->route('admin.onlinecourses.index')->with('error',$add_book_status);
				}   
				
			}else{
				return redirect()->route('admin.onlinecourses.index')->with('error', 'User Not Found');
			}					
		}else{
			return back()->with('error', "Access Denied");
		}
