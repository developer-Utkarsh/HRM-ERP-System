   ...
   public function download_pdf() {
	...
	public function editStartClass(Request $request){
		...
		if(empty($request->tt_id)){ 
		 return response(['status' => false, 'message' => 'Something Went Wrong'], 200);
			...
			if(empty($timetableData)){				
				$getRec = DB::table('timetables as t')
					->selectRaw("
						GROUP_CONCAT(DISTINCT b.id ORDER BY b.id SEPARATOR ',') as batch_ids,
						t.subject_id,
						b.master_planner
				...
				if($getRec->master_planner==0){
					$topics = DB::table('timetable_topic as tt')
					->selectRaw("
						tt.chapter_id,
						tm.name as chapter_name,
						tt.status,
						tt.batch_id,
						tt.subject_id
					")
					->leftJoin('chapter as tm', 'tm.id', '=', 'tt.chapter_id')
					->where('tt.status', 7)
					->where('tt.subject_id', $getRec->subject_id)
					->where('tt.batch_id', $getRec->batch_ids)
					->groupBy('tt.chapter_id', 'tm.name', 'tt.status', 'tt.batch_id', 'tt.subject_id')
					->get();
					
					foreach ($topics as $val) {
						$chapter_names[] = $val->chapter_name;
					...
					->selectRaw("
						...
						tt.subject_id
					")
					->leftJoin('topic_master as tm', 'tm.id', '=', 'tt.chapter_id')
					->where('tt.status', 7)
					->where('tt.subject_id', $getRec->subject_id)
					->where('tt.batch_id', $getRec->batch_ids)
					->groupBy('tt.chapter_id', 'tm.name', 'tt.status', 'tt.batch_id', 'tt.subject_id')
			...
			if(!empty($timetableData)){
				$topics = DB::table('timetable_topic as A')
					...
					->leftJoin('topic_master as chapter', 'chapter.id', '=', 'A.chapter_id')
					->where('A.is_comman', 1)
					...
					->groupBy('A.chapter_id','chapter.name')
					->get();
				...
				foreach ($topics as $val) {
					...
					$selectedPartial   = $val->status == 8 ? 'selected' : '';
					...
					$html .=	"<select class='form-control select-multiple1 ' name='chapter_id[".$timetableData->course_id."][]' style='width:100%;'>
					...
					$html .= "<div class='col-md-3 col-12'>
								<div class='form-label-group'>
									...
									<select class='form-control' name='status[".$timetableData->course_id."][]' style='width:100%;' required>
										...
										<option value='8' {$selectedPartial}>Dropped</option>
									</select>
								</div>
							  </div>";
					$html .= "<div class='col-md-1 col-12'>
			...
			if(empty($timetable) && count($timetable)==0){
				...
				foreach($timetable as $sch){
					...
					if($master_planner!=0){
						$tt_topic= DB::table('timetable_topic as A')->selectRaw("A.topic_id,A.status,chapter.id as chapter_id,chapter.name as chapter_name,topic.name as topic_name")
						->leftJoin("sub_topic_master as topic",'topic.id','A.topic_id')
						->leftJoin("topic_master as chapter",'chapter.id','A.chapter_id')
						->where('A.is_comman',0)
						->whereRAW($whereTopic)
						->groupBy('A.chapter_id')
						->get();
					}else{
						$tt_topic= DB::table('timetable_topic as A')->selectRaw("A.id,A.topic_id,A.status,chapter.id as chapter_id,chapter.name as chapter_name,topic.name as topic_name")
						->leftJoin("topic",'topic.id','A.topic_id')
						->leftJoin("chapter",'chapter.id','A.chapter_id')
						->where('A.is_comman',0)
						->whereRAW($whereTopic)
						->groupBy('A.chapter_id')
						->get();
					...
					if($course_planer_enable==1){
						...
						foreach($tt_topic as $val){
							...
							$html .="<div class='col-md-4 col-12 d-none'>";
							$html .="<div class='form-label-group'>";
							$html .="<span for='first-name-column'>Sub Topic - ".$val->topic_id."</span>";
							$t_topic_name .=$val->topic_name.",";
							$html .="<select class='form-control select-multiple1' name='topic_id[".$course_id."][]' style= 'width:100%;'>";
							...
							$selected=$val->status==1?'selected':'';
							$html .="<option value='1' $selected>Completed</option>";
							$selected=$val->status==8?'selected':'';
							$html .="<option value='8' $selected>Dropped</option>";
							$selected=$val->status==7?'selected':'';
							//$html .="<option value='7'>Schedule for nextclass</option>";
							$html .="</select>";
							$html .="</div>";
      ...
      $helper=new \App\Helper();
			...
			$chapter_name = implode(',', array_unique($chapter_names));
			
			if(Auth::user()->id!=7074 && Auth::user()->id!=8866){
				$html = '';
			}
	...
	public function updateStartClass(Request $request){ 
		...
		$spent_minutes = round($diff_seconds / 60);
		...
		if (!empty($request->topic_name)) {
			...
			foreach ($timetables as $val) {
				// Fetch all existing timetable_topic rows for this timetable & subject
				$rchapter_id = explode(',',$rchapter_id[0]);
				
				$existingTopics = TimetableTopic::where('timetable_id', $val->id)
					->where('batch_id', $val->batch_id)
					->where('subject_id', $val->subject_id)
					->whereIn('chapter_id', $rchapter_id)
					->pluck('chapter_id')
					->toArray();
		

				
				foreach ($rchapter_id as $chapter_id) {

					// Skip if already exists
					if (in_array($chapter_id, $existingTopics)) {
						continue;
					}

					$item = [
						'timetable_id' => $val->id,
						'topic_id'     => 0, // no topic here
						'chapter_id'   => $chapter_id,
						'status'       => 7,
						'batch_id'     => $val->batch_id,
						'subject_id'   => $val->subject_id,
		...
		//DB::table('timetables')->where('id', $timetable_id)->update(['is_publish' => '1', 'is_cancel' => '0', 'is_deleted' => '0','spent_minutes'=>$spent_minutes]);
		
		DB::table('timetables')->where(function($query) use ($timetable_id) {
			$query->where('id', $timetable_id)
				  ->orWhere('time_table_parent_id', $timetable_id);
		})->update(['is_publish' => '1','is_cancel' => '0','is_deleted' => '0','spent_minutes' => $spent_minutes]);
	...
	public function scheduleNextTopic(Request $request){
		...
		$master_planner_id	=	$request->master_planner_id;
		
		
		if (!empty($request->master_planner_id)) {
			$master_planner_ids = explode(',', $request->master_planner_id);
		} else {
			$master_planner_ids = [0]; // fallback single iteration with no master_planner
		}

		
		$rsp=['status'=>false,'message'=>'Something Went Wrong!'];
		 
		if($action=='schedule' || $action=='completed' || $action=='dropped'){
			$batch_ids=[];
			...
			if ($action == 'completed' || $action == 'dropped') {
				for($i=0;$i<count($batch_ids);$i++){
					...
					foreach ($master_planner_ids as $mp_id_raw) {
						$master_planner_id = (int) trim($mp_id_raw);

						$query = DB::table('timetable_topic')
							->where('batch_id', $batch_id)
							->where('subject_id', $subject_id)
							//->where('chapter_id','!=', $chapter_id)
							->whereIN('status', [0,2,7]);

						if (count($batch_ids) > 1 && $master_planner_id != 0) {
							$query->where('is_comman', 1)
								  ->where('master_planner_id', $master_planner_id);
						}else {
							$query->where('master_planner_id', 0);
						}						
			...
			for ($i = 0; $i < count($batch_ids); $i++) {
				$batch_id = (int) trim($batch_ids[$i]);
				
				if(count($batch_ids) > 1){
					$master_planner_id = isset($master_planner_ids[$i]) ? (int) trim($master_planner_ids[$i]) : 0;
				}else{
					$master_planner_id = 0;
				}
				...
				$data = [
					...
					'source'       => 'FacultyPage',
					'status'       => 7,
					'master_planner_id' => $master_planner_id,
					'is_comman'    => (count($batch_ids) > 1 && $master_planner_id > 0) ? 1 : 0,
				];
				...
				if ($exists) {
					TimetableTopic::whereIN('status',[0,2,7,8])->where('id', $exists->id)->update($data);
					$rsp = ['status' => true, 'message' => 'Updated Successfully!'];
				} else {
					TimetableTopic::insert($data);
					$rsp = ['status' => true, 'message' => 'Inserted Successfully!'];
				}
			}
	...
	public function get_topic_class(Request $request)
	{
		...
		$getClass = DB::table('timetable_topic as tt')
			->leftjoin('start_classes as sc','sc.timetable_id','tt.timetable_id')
			->where('tt.chapter_id', $chapterId)
		...
		if ($getClass->count() > 0) {
			...
			foreach($getClass as $gc){
				$startTime = strtotime($gc->start_time);
				$endTime   = strtotime($gc->end_time);
				...
				if ($startTime && $endTime) {
					...
					if ($hours > 0) {
						$diff = sprintf('%dh %02dm', $hours, $minutes);
					} else {
				...
				$res .= '<tr>
						<td>Part-'.$this->roman_number($i).'</td>
						<td>'.date('d-m-Y',strtotime($gc->sc_date)).'</td>
						<td>'.$gc->start_time.'</td>
						<td>'.$gc->end_time.'</td>
						<td>'.$diff.'</td>
				</tr>';
			...
			$res .= '<tr><td colspan="5" class="text-center">No Class Found</td></tr>';
		}

		return response(['status'=>true,"html"=>$res]);
	}
	
	
	public function roman_number($num)
	{
		$n = intval($num);
		$res = '';
