...
class FreeFacultyReportsController extends Controller
{
    ...
    public function index()
    {
       ...
       // $tdate      = Input::get('tdate');
		...
		$get_timetables->where('timetables.is_deleted', '=', '0');
		$available_faculty = [];
		$get_timetables = $get_timetables->get();
		if(count($get_timetables) > 0){
			foreach($get_timetables as $key=>$timetableVal){
				$available_faculty[$key]['faculty_id'] = $timetableVal->faculty_id;
				$available_faculty[$key]['from_time']  = $timetableVal->from_time;
				$available_faculty[$key]['to_time']    = $timetableVal->to_time;
			}
		}
        ...
        return view('admin.free_faculty_reports.index', compact('faculty_record','fdate'));
    }
