...
class AttendanceRecordSingleController extends Controller
{
	
	public function attendencerecordsingle(Request $request){
		$mt = date('m');
		//$mt="02";
		$yr = date('Y');
		...
		return view('admin.attendance.attendancerecordsingle', compact('allDepartmentTypes','allBranches','getWorkSunday'));
	}
	
	public function attendencerecorddetailsingle(Request $request){		
		$logged_role_id      = Auth::user()->role_id;
		$logged_id           = Auth::user()->id;
		$name                = $request->name;
	...
	public function calculate_attendance($logged_role_id,$logged_id,$name,$department_type,$branch_id,$year_wise_month,$status=NULL){
		...
		if($status != ''){
			...
			$CurrentMonth = date('n',strtotime($year_wise_month));
			$new_whereCond .= " AND (users.status = '1')";
		}
		
		
		$CurrentMonth = date('n',strtotime($year_wise_month));
		$new_whereCond .= " AND (users.is_deleted = '0')";
		...
		$new_whereCond .= " AND (userdetails.joining_date < '$get_date_gete')";
		...
		$arrayDate =array(0=>'2021-08-30',1=>'2021-10-13',2=>'2021-11-19',3=>'2021-12-25');
		...
		if(count($array1) > 0){
			foreach($array1 as $val){
				...
				$i = 1;
				$date_array    = array();
				foreach($arrayDate as $get_date_gete){
					$first_date = strtotime("$get_date_gete");
					$last_date = strtotime("$get_date_gete");
					$user_id = $val->id;
					$leave_balance = 0;				
					...
					if($val->is_extra_working_salary=="1"){
						$actual_paid = $actual_paid + $total_holiday_working;
					}
				}
