...
class MultiCoursePlannerController extends Controller
{
	...
	public function subject_assign(Request $request, $id){
		...
		$subject = DB::table('subject as s')
					...
					->join('sub_topic_master as stm', 'stm.topic_id', '=', 'tm.id')
					->where('s.status', 1)
					->select('s.id', 's.name')
					->groupBy('s.id', 's.name')
					->get();
	...
	public function multiplannersummary(Request $request,$req_id){
		...
		if($role_id==29 || $department_type==13 || ($department_type==50 && $logged_id!=8232 && $logged_id!=8866)){
			$record	= array();
			$topic	= array();
			// $topic_relation = DB::table('course_planner_topic_relation as cptr')
				...
				// ->leftJoin('course_planner_sme_relation as cpsr', 'cpsr.req_id', '=', 'cptr.req_id')
			...
			/*
			$topic_relation_query = DB::table('course_planner_topic_relation as cptr')
				->leftJoin('planner_request as pr', 'pr.id', '=', 'cptr.req_id');

			if ($role_id != 21 || $logged_id != 8232) {
				$topic_relation_query->leftJoin('course_planner_sme_relation as cpsr', function ($join) use ($subjectIds) {
					$join->on('cpsr.req_id', '=', 'pr.id')
						 ->where('cpsr.sme_id', Auth::user()->id)
						 ->whereIn('cpsr.subject_id', $subjectIds);
				});
			}
			
			$topic_relation = $topic_relation_query
				->leftJoin('topic_master as tm', 'tm.id', '=', 'cptr.topic_id')
				->leftJoin('sub_topic_master as stm', 'stm.id', '=', 'cptr.sub_topic_id')
				->leftJoin('subject as sm', 'sm.id', '=', 'tm.subject_id')
			...
			if ($role_id != 21 && $logged_id != 8232) {
				$topic_relation->whereIn('tm.subject_id', $subjectIds);
			}
			
			$topic_relation= $topic_relation->select(
					'cptr.*',
					'tm.name as topic_name',
					'stm.name as sub_topic_name',
					'sm.name as subject_name',
	...
	public function topic(Request $request){
		$name	=	$request->name;
		$status	=	$request->status;
		$record = DB::table('topic_master')->select('topic_master.*','subject.name as subject_name')->leftjoin('subject','subject.id','topic_master.subject_id')->orderby('id','desc');
		...
		$record = $record->get();
		return view('admin.multi-course-planner.topic',compact('record'));
	}
	...
	public function edit_topic($id){
		...
		return view('admin.multi-course-planner.add_topic',compact('record'));
	}	
	...
	public function save_topic_master(Request $request){
		...
		if(!empty($tname)){
			if(!empty($tid)){
				DB::table('topic_master')->where('id',$tid)->update(['subject_id'=>$subject_id,'name'=>$tname]);
				return redirect()->back()->with('success', 'Topic Updated!!');

			}else{
				DB::table('topic_master')->insert(['subject_id'=>$subject_id,'name'=>$tname]);
	...
	public function edit_sub_topic($id){
		...
		return view('admin.multi-course-planner.add_sub_topic',compact('record','id'));		
	}
