...
class BatchReportsController extends Controller
{
    ...
    { 
		...
		if(!empty(Auth::user()->id)){
			return view('admin.batch_reports.without_login_assistant_report', compact('get_studios','fdate'));
		}
		else{
			return view('admin.batch_reports.without_login_assistant_report', compact('get_studios','fdate'));
		}
    ...
    { 
		...
		$get_batches = Batch::with(['batch_timetables'=> function ($q) use ($fdate, $tdate, $online_class_type)
		{
		...
		if (!empty($batch_id)) {
			$get_batches = $get_batches->whereIn('id', $batch_id);
		}
		
		
		if (!empty($erp_id))
		{
			$get_batches = $get_batches->where('erp_course_id', $erp_id);
		}
		...
		if(!empty(Auth::user()->id)){
			return view('admin.batch_reports.index', compact('get_batches','fdate','tdate'));
		}
		else{
			return view('admin.batch_reports.without_login_batch_report', compact('get_batches','fdate','tdate'));
		}
   ...
   public function download_pdf() { 
		...
		if (!empty($batch_id)) {
			$get_batches->whereIn('id', $batch_id);
		}
		...
		//$get_batches = $get_batches->skip(0)->take(5)->get();
		$get_batches = $get_batches->get();
		//echo json_encode($get_batches);die;
		//echo '<pre>'; print_r($get_batches);die;
		return view('admin.batch_reports.pdf_html', compact('get_batches','fdate','tdate'));
        ...
        // ---------------------------------------------------------
        // $title = 'Studio Report';
		$html = view('admin.batch_reports.pdf_html', compact('get_batches'))->render();
        //echo $html; //exit();
    ...
    { 
		...
		//echo '<pre>'; print_r($get_batches);die;
		if(!empty(Auth::user()->id)){
			return view('admin.batch_reports.batch_reports_shift_wise', compact('get_batches','fdate','tdate','branch_id','type','batch_id'));
		}
	...
	public function download_pdf_shiftwise(){
		...
		$get_batches = Batch::select('id','name')->where('status','1')->where('is_deleted','0')->get();
		//echo '<pre>'; print_r($get_batches);die;
		return view('admin.batch_reports.shiftwise_pdf_html', compact('get_batches','fdate','tdate','branch_id','type','batch_ids'));
	...
	public function studio_availability(){
       ...
       $branch_id    = Input::get('branch_id');
	    ...
	    return view('admin.batch_reports.studio_availability',compact('get_studios','cdate'));	
	}
    ...
    { 
		...
		return view('admin.batch_reports.batch_hours_report', compact('batch_id','batch_details'));
