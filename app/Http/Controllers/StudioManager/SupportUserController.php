...
class SupportUserController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('studiomanager.support_user.index', compact('support_user','category','users'));
    }
    ...
    {  
		...
		if ( $validation->fails() ) {
			return redirect()->route('studiomanager.support_user.index')->with('error', 'Employee Already Exits');
		}
        ...
        if($res) {
            return redirect()->route('studiomanager.support_user.index')->with('success', $msg);
        } else {
            return redirect()->route('studiomanager.support_user.index')->with('error', 'Something Went Wrong !');
        }
    }
	...
	public function edit_support_user(Request $request){ 
			...
			<input type="hidden" name="id" value="'.$respnse->id.'">
				...
				<div class="col-md-12 mb-2">
					<select class="form-control role select-multiple1" name="role" required>
						...
						<option value="studiomanager"';if(!empty($respnse->role) && $respnse->role == 'studiomanager'){ $res .='selected'; } $res .='>studiomanager</option>
					</select><br>
				</div>
        ...
        {
           return redirect()->route('studiomanager.support_user.index')->with('error', 'Data Not Found');
        }
	}
    ...
    public function togglePublish($id) { 
        ...
        if (is_null($support_user)) {
            return redirect()->route('studiomanager.support_user.index')->with('error', 'Support User not found');
        }
        ...
        return redirect()->route('studiomanager.support_user.index')->with('success', 'Status Updated Successfully.');
    }
}
