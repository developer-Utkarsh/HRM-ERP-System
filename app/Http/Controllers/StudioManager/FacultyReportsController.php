...
class FacultyReportsController extends Controller
{
    ...
    public function index()
    {
		...
		if(!empty($subject_id)){
			$whereCond .= ' AND timetables.subject_id = "'.$subject_id.'"';
		}
		
		if(!empty($category_id)){
			$whereCond .= ' AND batch.category = "'.$category_id.'"';
		}
		...
		if(!empty($search)){
			$get_faculty = DB::table('timetables')
			   ->select('timetables.id','timetables.assistant_id','timetables.faculty_id','users_faculty.name as faculty_name','studios.name as studio_name')
        ...
        return view('studiomanager.faculty_reports.index', compact('get_faculty','selectFromDate','selectToDate','status','branch_id','batch_id','type','assistants','get_category','category_id'));
    }
                    ...
                    if($course_planer_enable==1){
					  ...
					  $html .= "<pre class='rounded text-dark'><div class='col-6 p-2'><span class='fa fa-right-o'></span>".$sch->course_name."</div></pre>";
		                ...
		                if(count($get_chapter)){
							...
							$html .="<div class='form-label-group'>";
							$html .="<span for='first-name-column'>Topic Name</span>";
							$html .="<select class='form-control chapter_data select-multiple1' name='chapter_id[".$course_id."][]' style= 'width:100%;'>";
							...
							$html .="<select class='form-control topic_data' name='topic_id[".$course_id."][]' style= 'width:100%;'>";
							...
							$html.="<div class='col-md-1 col-12 disabled_btn'>
			           ...
			           <i class='fa fa-minus' aria-hidden='true'></i></span>
							...
							$html.= "<div class='row-- col-12--'>
							<div class='text-right' style='position:absolute;margin-top:-60px;margin-left:-20px;float:right;'>
...
if(!empty($selectFromDate) && !empty($branch_location)){
	...
	public function faculty_topic(Request $request){
		...
		// return view('admin.faculty_reports.faculty_topic',compact('cards','faculty'));
