...
class FacultyLeaveController extends Controller
{
	...
	public function faculty_leave()
    {
		...
		$faculties = DB::table('faculty_leave')
			...
			if(!empty(Input::get('fdate'))){
				// $faculties->whereRaw("faculty_leave.reason !=''");
			}
		$faculties = $faculties->whereRaw("date(faculty_leave.date) >= '$fdate' and date(faculty_leave.date) <= '$tdate'");
		$get_data = $faculties->get();
		
		$get_faculty = DB::table('users')->select('id','name','mobile')->where('role_id',2)->where('is_deleted','0')->get();
        return view('studiomanager.faculty_leave.index', compact('get_data','get_faculty'));
    }
	...
	public function faculty_leave_download()
    {
        ...
        return view('studiomanager.faculty_leave.faculty_leave_pdf', compact('get_data','fdate'));
    }
	...
	public function faculty_leave_add(){
		...
		$users        = User::where('role_id', 2)->where('status', 1)->where('is_deleted', '0')->get();
		return view('studiomanager.faculty_leave.add', compact('users'));
	}
