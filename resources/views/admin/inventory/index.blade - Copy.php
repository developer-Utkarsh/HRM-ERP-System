		...
		<div class="content-header row">
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-md-8">
						<h2 class="content-header-title float-left mb-0">Product Inventory List</h2>
					...
					<div class="col-md-4"> <a href="{{ route('admin.inventory.index') }}" data-id="" class="btn btn-outline-primary float-right">Back</a></div>
				</div>
			</div>
		</div>
				...
				<div class="table-responsive">
					<table class="table data-list-view">
						...
						@if(count($product) > 0)
							...
							@php
							$total_qty   = DB::table('inventory')->where('id', $value->id)->where('is_parent', '0')->sum('qty');
							//->orWhere('is_parent', $value->id)
							$total_price = DB::table('inventory')->where('id', $value->id)->where('is_parent', '0')->sum('price');
							//->orWhere('is_parent', $value->id)
							...
							<tr>
								<td>{{ $key + 1 }}</td>
								<td class="product-category">{{ $value->p_name }}</td>
								<td class="product-category">{{ $value->buyer_name }}</td>
								<td class="product-category">{{ $total_qty }}</td>
								<td class="product-category">{{ $rem_pro }}</td>
								<td class="product-category">{{ $total_price }}</td>
								<td>{{ $value->created_at->format('d-m-Y') }}</td>
								<td class="product-action">
									<a title="Update Inventory" href="{{ route('admin.inventory.edit', $value->id) }}">
										<span class="action-edit"><i class="feather icon-edit"></i></span>
									</a>
									<!--a title="Transfer" href="{{ route('admin.inventory.inventory-transfer', [$value->product_id, $value->id]) }}">
										<span class="action-transfer"><i class="feather icon-repeat"></i></span>
									</a>
									<a title="Transfer List" href="{{ route('admin.inventory.inventory-transfer-list', [$value->product_id, $value->id]) }}">
