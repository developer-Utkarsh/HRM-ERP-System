			...
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					...
					<div class="col-6">
						...
						<a href="{{ route('admin.enquiry.create') }}" class="btn btn-primary" style="float: right;">Add Enquiry</a>
						@endif
					</div>
				</div>
			</div>
					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.enquiry.index') }}" method="get" name="filtersubmit" id="filtersubmit">
									<div class="row">
											...
											<fieldset class="form-group">												
												<select class="form-control" name="category_id"> 
													...
													@foreach($category as $categoryvalue)
														<option value="{{ $categoryvalue->id }}" @if(!empty(app('request')->input('category_id')) && app('request')->input('category_id') == $categoryvalue->id){{"selected"}}@endif >{{ $categoryvalue->name }}</option>
													@endforeach
													@endif
												</select>   												
											</fieldset>
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('admin.enquiry.index') }}" class="btn btn-warning">Reset</a>
											</fieldset>
					...
					<table class="table data-list-view" id="TableSearch">
						...
						@if(count($enquiry_result) > 0)
							...
							<tr>
								...
								@if($check_add_enquiry->role == 'admin' && $enquiry_result_value->query_id ==  Auth::user()->id)
									<a href="{{route('admin.enquiry.edit',$enquiry_result_value->id)}}" class="btn btn-outline-info btn-sm mt-1" data-id="{{$enquiry_result_value->id}}" title="Edit Enquiry" style="padding: 0.5rem 0.5rem;"><i class="fa fa-pencil"></i></a>
								@endif	
								
								@if($enquiry_result_value->date == date('Y-m-d') && $enquiry_result_value->query_id ==  Auth::user()->id)
									<a href="{{route('admin.enquiry.destroy',$enquiry_result_value->id)}}" class="btn btn-outline-info btn-sm mt-1" data-id="{{$enquiry_result_value->id}}" title="Delete Enquiry" style="padding: 0.5rem 0.5rem;" onclick="return confirm('Are you sure want to delete?')"><i class="fa fa-remove"></i></a>
								@endif
...
<div id="apply-form" class="modal fade">
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="{{ route('admin.store-enquiry-description') }}">
...
<div id="status-form" class="modal fade">
	<div class="modal-dialog modal-lg">
		...
		<form method="post" action="{{ route('admin.store-enquiry-status') }}">
			<div class="modal-body">
				<div class="form-body">
					@csrf
					<div class="row"> 
