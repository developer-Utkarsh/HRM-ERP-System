					...
					<div class="col-12">
						<div class="card">
							<div class="card-content">
								<div class="card-body">
									<form class="form" action="{{ route('admin.batch_holiday.update', $holiday->id) }}" method="post" enctype="multipart/form-data">
										<!-- @method('PATCH')-->
										@csrf
										<div class="form-body">
											<div class="row">
												...
												<div class="col-12 col-md-4 branch_loader">
													...
													<fieldset class="form-group">	
														...
														<select class="form-control select-multiple2 branch_location" name="branch_location" id="">
															...
															<option value="{{ $value->name }}" @if((!empty(old('branch_location')) && $value->id == old('branch_location')) || (!empty($holiday->location))) selected="selected" @endif>{{ $value->name }}</option>
															@endforeach
															@endif
														</select>
													</fieldset>
												...
												<div class="col-12 col-md-4 branch_loader">
													...
													<fieldset class="form-group">												
														<select class="form-control select-multiple1 course_id" name="course_id" id="">
															...
															<option value="{{ $value->id }}"  @if((!empty(old('batch_id')) && $value->id == old('batch_id')) || (!empty($holiday->course_id))) selected="selected" @endif>{{ $value->name }}</option>
															@endforeach
															@endif
														</select>
												...
												<div class="col-12 col-md-4 branch_loader">
													...
													<fieldset class="form-group">												
														<select class="form-control select-multiple1 batch_id" name="batch_id[]" id="" multiple>
															...
															<option value="{{ $value->id }}"  @if((!empty(old('batch_id')) && $value->id == old('batch_id')) || (!empty($holiday->batch_id) && in_array($value->id, json_decode($holiday->batch_id)))) selected="selected" @endif>{{ $value->name }}</option>
															@endforeach
															@endif
														</select>
														@if($errors->has('batch_id'))
