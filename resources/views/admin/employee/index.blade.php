					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.employees.index') }}" method="get" name="filtersubmit">
									<div class="row">
										<div class="col-md-3">
										...
										<div class="col-md-3">
											...
											<fieldset class="form-group">
												<input type="text" class="form-control darwin_code" name="darwin_code" placeholder="Darwin Code" value="{{ app('request')->input('darwin_code') }}" id="myInputSearch">
											</fieldset>
										</div>
										<div class="col-md-3">
										...
										<div class="col-md-2">
											...
											<fieldset class="form-group">
												<input type="month" class="form-control year_wise_month" name="year_wise_month" value="@if(!empty(Request::get('year_wise_month'))){{ Request::get('year_wise_month') }}@endif">
											</fieldset>
										</div>
							...
							@if(count($employees) > 0)
								...
								<tr>
									...
									<td class="product-name">
										<a href="{{ route('admin.employees.view', $value->id) }}" class="btn btn-sm btn-primary">{{ $value->register_id }}
										</a>
									</td>
									<td>{{ $value->darwin_code ?? '-' }}</td>
									<td>{{ $value->email }}</td>
									<td class="product-price">{{ $value->mobile }}</td>
									<td class="product-price">{{ isset($value->role->name)?$value->role->name:'' }}</td>
									<?php if( Auth::user()->role_id ==29 || Auth::user()->role_id ==30 || Auth::user()->role_id ==24){?>
									<td class="product-price">{{ !empty($value->user_details->joining_date) ? date('d-m-Y',strtotime($value->user_details->joining_date)) : '' }}</td>
									<td class="product-price">{{ !empty($value->reason_date) ? date('d-m-Y',strtotime($value->reason_date)) : '' }}</td>
									<?php } ?>
									@if(Auth::user()->role_id == 29 || Auth::user()->role_id == 24)
									<td>
										{{-- @if($value->status == "1")
										<a href="{{ route('admin.employee.status', $value->id) }}"><i class="fa fa-toggle-on"></i> Active </a>
										@else
										<a href="{{ route('admin.employee.status', $value->id) }}"><i class="fa fa-toggle-off"></i> Inactive </a>
										@endif --}}
										<!--a href="{{route('admin.employee.status', $value->id)}}" @if(Auth::user()->role_id == 21)style="display: inline-block;pointer-events: none;" @endif>
										...
										<a href="@if($value->status == 1){{ 'javascript:void(0)' }} @else {{route('admin.employee.status', $value->id)}} @endif" <?php if($value->status == 1){ echo 'data-toggle=modal data-id='.$value->id.' data-status='.$value->status ; } ?> id="lnk" @if(Auth::user()->role_id == 21)style="display: inline-block;pointer-events: none;" @endif>
									...
									?>
										<a href="{{ route('admin.employees.approval', $value->id.$nCheck) }}">
									...
									if(Auth::user()->role_id == 29 || Auth::user()->role_id == 24){
									?>
										<a href="{{ route('admin.employees.is-comp-off', $value->id.$nCheck) }}" title="Is Com Off">
											<span class="action-edit"><i class="feather icon-scissors"></i></span>
										</a>
									<?php 
									}
									?>
										<a href="{{ route('admin.employees.view', $value->id.$nCheck) }}"  title="Employee Details">
											<span class="action-edit"><i class="feather icon-eye"></i></span>
										</a>
										@if(Auth::user()->role_id == 24 || Auth::user()->role_id == 29 || Auth::user()->role_id == 30)
										<a href="{{ route('admin.employee.edit', $value->id.$nCheck) }}" title="Edit Employee">
											<span class="action-edit"><i class="feather icon-edit"></i></span>
										</a>
										<!--
										<a href="{{ route('admin.employee.delete', $value->id) }}" onclick="return confirm('Are You Sure To Delete Employee')" title="Delete Employee">
