					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.freelancer.faculty-invoice-history') }}" method="get" name="filtersubmit">
									<div class="row">
										<div class="col-12 col-sm-6 col-lg-3">
											<label for="users-list-role">Month</label>
											<fieldset class="form-group">
												<input type="month" class="form-control" name="month" placeholder="Month" value="{{ app('request')->input('month', date('Y-m')) }}">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">
												<select class="form-control faculty_id select-multiple1" name="faculty_id">
													...
													<option value="{{ $fa->id }}" 
														{{ app('request')->input('faculty_id') == $fa->id ? 'selected' : '' }}>
														{{ $fa->name }}
													</option>
													<?php } ?>
												</select>
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('admin.freelancer.faculty-invoice-history') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
					...
					<table class="table data-list-view" id="">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $r->month }}</td>
								<td>
									<a href="{{ asset('laravel/public/invoice/' . $r->invoice) }}" download>
										Preview
									</a>
								</td>
								<td>
								...
								<td>
									<?php if($r->status==0){ ?>
									<button type="button" class="btn btn-primary btn-sm updateStatus" data-id="{{ $r->id }}">
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form action="{{ route('admin.freelancer.invoice-stauts-update') }}" method="post">
		@csrf
