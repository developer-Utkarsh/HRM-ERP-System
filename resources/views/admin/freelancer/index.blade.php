			...
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					...
					<div class="col-6 text-right">
						<a href="{{ route('admin.freelancer.create') }}" class="btn btn-primary">Add</a>
						
						<a href="{{ route('admin.freelancer.freelancer-invoice-history') }}" class="btn btn-primary">History</a>
					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.freelancer.index') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('admin.freelancer.index') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
						...
						<tbody>
							...
							<tr>
								...
								<td>
									...
									<select class='form-select status-dropdown d-none' data-id="{{ $r->id }}">
...
<script type="text/javascript">
	...
	$(document).on('change', '.status-dropdown', function () {
		...
		$.ajax({
			type : 'POST',
			url : '{{ route('admin.freelancer.fchange-status') }}',
			data : {'_token' : '{{ csrf_token() }}', 'id': id,'status':newStatus},			
	...
	$(document).ready(function () {
		$('#submitInvoice').click(function (e) {
			...
			if (isValid) {
				$.ajax({
					url : '{{ route('admin.freelancer.freelancer-invoice') }}',
					type: 'POST',
					data : {
						'_token' : '{{ csrf_token() }}',
						'name': name,
					...
					success: function (response) {
						...
						if (data.status === 'success') {
							...
							window.location.href = "{{ route('admin.freelancer.freelancer-invoice-download', '/insert_id') }}".replace('/insert_id', data.insert_id);
