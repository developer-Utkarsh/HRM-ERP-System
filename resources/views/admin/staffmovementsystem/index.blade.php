			...
			<section id="data-list-view" class="data-list-view-header">
							...
							<div class="users-list-filter">
								<form action="{{ route('admin.staff.index') }}" method="get">
									<div class="row">
										...
										<input type="hidden" class="faculty_id_get" value="{{ app('request')->input('faculty_id') }}">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple1 emp_id" name="emp_id">
													...
													<option value="{{ $value->id }}" @if($value->id == app('request')->input('emp_id')) selected="selected" @endif>{{ $value->name." (".$value['register_id'].")" }}</option>
													@endforeach
													@endif
												</select>												
											</fieldset>
										...
										<div class="col-12 col-sm-6 col-lg-5">	
											...
											<a href="{{ route('admin.staff.index') }}" class="btn btn-warning">Reset</a>
				...
				<?php 
						...
						?>
							...
							<td>
						 <button type="button" class="p-0 btn btn-outline-primary change_sts" data-edit="{{$staff_report_value->id}}" oNChn>Change Status</button>
								</td>
							</tr>
...
$(document).ready(function() {
	...
	$("body").on("click", ".change_sts", function (e) {
		...
		$("#my_url").attr("action","");
		$("#my_url").attr("action","{{ url('admin/staff/update/') }}/"+edit_id);
