					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.faculty-sme-uploadfile-submit') }}" method="post" name="filtersubmit" enctype='multipart/form-data'>
									@csrf
									<input type="hidden" name="reuqest_id" value="{{ $request_id }}"/>
									<div class="row">
										...
										<div class="col-lg-4 form-group">
											...
											<select name="exam[]" class="select-multiple form-control select-multiple select_exam" multiple required>
												...
												<?php 
													...
													foreach($exam as $ex){												
												?>
													<option value="<?=$ex->name."$#".$ex->id?>" data-id="{{ $ex->id }}" selected>{{ $ex->name }}</option>
												<?php } } ?>
											</select>
										</div>
										<div class="col-lg-4 form-group">
											...
											<select name="subject[]" class="select-multiple form-control select_subject" multiple required>
												...
												?>
													<option value="<?=$su->name."$#".$su->id?>" data-id="{{ $su->id }}" selected>{{ $su->name }}</option>
												<?php } } ?>
											</select>
										</div>
										<div class="col-lg-4 form-group">
											...
											<select name="chapter[]" class="select-multiple form-control select_chapter" multiple required>
												...
												<?php 
													...
													foreach($topic as $to){												
												?>
													<option value="<?=$to->name."$#".$to->id?>" data-id="{{ $to->id }}" selected>{{ $to->name }}</option>
												<?php } } ?>
											</select>
										</div>
										<div class="col-lg-4 form-group">
										...
										<div class="col-lg-4 form-group">
											...
											<select name="mode" class="form-control" required onchange="getMode(this.value)">
												<option value="">Select</option>
												<option value="Manual" {{ (!empty($get_details->mode) && $get_details->mode == 'Manual') ? 'selected' : '' }}>Manual</option>
												<option value="PrashnKosh" {{ (!empty($get_details->mode) && $get_details->mode == 'PrashnKosh') ? 'selected' : '' }}>PrashnKosh</option>
											</select>
										</div>
										...
										<div class="col-lg-4 form-group">
											...
											<select name="level[]" class="select-multiple form-control" multiple required>
												<option value="">Select</option>
												<option value="Easy" {{ (!empty($get_details->level) && in_array('Easy', explode(',', $get_details->level))) ? 'selected' : '' }}>Easy</option>
												<option value="Medium" {{ (!empty($get_details->level) && in_array('Medium', explode(',', $get_details->level))) ? 'selected' : '' }}>Medium</option>
												<option value="Hard" {{ (!empty($get_details->level) && in_array('Hard', explode(',', $get_details->level))) ? 'selected' : '' }}>Hard</option>
											</select>
										...
										<div class="col-lg-4 form-group">
											...
											<select name="requirement_for" class="form-control" required>
												<option value="">Select</option>
												<option value="YouTube" {{ (!empty($get_details->requirement_for) && $get_details->requirement_for == 'YouTube') ? 'selected' : '' }}>YouTube</option>
												<option value="Offline Batch" {{ (!empty($get_details->requirement_for) && $get_details->requirement_for == 'Offline Batch') ? 'selected' : '' }}>Offline Batch</option>
												<option value="Online Batch" {{ (!empty($get_details->requirement_for) && $get_details->requirement_for == 'Online Batch') ? 'selected' : '' }}>Online Batch</option>
											</select>
										</div>
										<div class="col-lg-4 form-group">
											...
											<select name="language" class="form-control" required>
												<option value="">Select</option>
												<option value="English" {{ (!empty($get_details->language) && $get_details->language == 'English') ? 'selected' : '' }}>English</option>
												<option value="Hindi" {{ (!empty($get_details->language) && $get_details->language == 'Hindi') ? 'selected' : '' }}>Hindi</option>
												<option value="Bilingual" {{ (!empty($get_details->language) && $get_details->language == 'Bilingual') ? 'selected' : '' }}>Bilingual</option>
											</select>
										</div>
										<div class="col-lg-12 form-group">
											...
											<br>
											<iframe src="{{ asset('laravel/public/faculty_sme/'.$get_details->file) }}" width="100%" height="400px" style="border: none;"></iframe>
											<?php } ?>
										</div>
										
										<?php if($viewonly!=1){ ?>
										<div class="col-12 col-sm-12 col-lg-12">
											<input type="hidden" name="resue_by" value="{{ $resue_by }}"/>
											<label for="users-list-role">&nbsp;</label>
											<fieldset class="form-group" style="float:right;">		
											<button type="submit" class="btn btn-primary">Submit</button>
											<a href="{{ route('admin.faculty-sme-uploadfile') }}?request_id={{ $request_id }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
										<?php } ?>
									</div>
