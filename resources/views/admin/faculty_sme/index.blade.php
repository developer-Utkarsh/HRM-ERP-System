					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.faculty-sme.index') }}" method="get" name="filtersubmit">
									<div class="row">
										<div class="col-12 col-sm-6 col-lg-3">
											<label for="users-list-role">Request ID</label>
											<fieldset class="form-group">
												<input type="text" class="form-control" name="request_id" placeholder="Request ID" value="{{ app('request')->input('month') }}">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">
												<select class="form-control req_status" name="req_status">
													...
													@foreach ([0 => 'Pending', 1 => 'Completed', 2 => 'Rejected', 3 => 'Picked'] as $value => $label)
														<option value="{{ $value }}" {{ request('req_status') == $value ? 'selected' : '' }}>
															{{ $label }}
														</option>
													@endforeach
												</select>
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('admin.faculty-sme.index') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
					...
					<table class="table data-list-view" id="">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $re->reason??'-' }}</td>
								<td class="actionBtn">
									<a href="{{ route('admin.faculty-sme.faculty-sme-chat') }}?request_id={{ $re->request_id }}" title="Chat" class="text-success"><i class="fa fa-commenting-o" aria-hidden="true"></i>
									...
									<?php if($re->status==0){ ?>
									<a href="{{ route('admin.faculty-sme.faculty-sme-request-pick') }}?request_id={{ $re->request_id }}" class="text-warning" title="Pick Task"><i class="fa fa-check" aria-hidden="true"></i></a>
										...
										<?php 
											if(empty($re->assistant_used)){
												// if($re->status==0 || $re->status==3){ 
													if(empty($re->file)){ 
										?>
										<a href="{{ route('admin.faculty-sme-uploadfile') }}?request_id={{ $re->request_id }}" title="Upload File"><i class="fa fa-upload" aria-hidden="true"></i></a>
										<?php }else{ ?>
										<a href="{{ route('admin.faculty-sme-uploadfile') }}?request_id={{ $re->request_id }}" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
										<?php 
													} 
												// }
											}else{
										?>
											<a href="{{ route('admin.faculty-sme-uploadfile') }}?request_id={{ $re->request_id }}&viewonly=1" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a>
										<?php } ?>
										
										<?php if(empty($re->file)){ ?>
										<a href="{{ route('admin.faculty-sme.faculty-sme-reuse') }}?request_id={{ $re->request_id }}" class="text-warning"><i class="fa fa-repeat" aria-hidden="true" title="Reuse"></i></a>
										<?php } ?>									
										
										
									
										<?php if($re->status==0 || $re->status==3){ ?>
										<a href="javascript:void(0)" class="status-model text-danger" data-id="{{ $re->id }}"><i class="fa fa-toggle-on" aria-hidden="true"></i></a>
										<?php } ?>
...
<div class="modal fade" id="exampleModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
		...
		<form action="{{ route('admin.faculty-sme.faculty-sme-update-stauts') }}" method="post">
			@csrf
