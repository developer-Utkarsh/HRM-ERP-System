									...
									<div class="row">
									    ...
									    <div class="col-md-3">
											<label for="users-list-role"></label>
											 <input type="text" class="form-control" placeholder="Enter OTP" name="otp" value="@if(!empty(app('request')->input('otp'))){{app('request')->input('otp')}}@endif">
										</div>
...
<script type="text/javascript">
	...
	$("#generate_salary_otp").submit(function(e) {
		...
		$.ajax({
			...
			type: "POST",
			url : '{{ route('admin.salary-send-otp') }}',
			data : {},
			processData : false, 
			contentType : false,
			dataType : 'json',
	...
	$("#check_salary_otp").submit(function(e) {
		...
		$.ajax({
			...
			type: "POST",
			url : '{{ route('admin.salary-access-otp') }}',
			data : dataForm,
			processData : false, 
			contentType : false,
			dataType : 'json',
