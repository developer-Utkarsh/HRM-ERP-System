					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.request.reports.inventory-valuation') }}" method="get" name="filtersubmit">
									<div class="row pt-2">
										<div class="col-12 col-sm-6 col-lg-3">
											<fieldset class="form-group">
												<label for="users-list-status">Month</label>
												<input type="month" class="form-control" name="fmonth" placeholder="Title" value="{{ app('request')->input('fmonth') }}">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-3">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple asset_type" name="asset_type">
													<option value="">Select Any</option>
													<option value="Asset" @if('Asset' == app('request')->input('asset_type')) selected="selected" @endif>Asset</option>
													<option value="Non Asset" @if('Non Asset' == app('request')->input('asset_type')) selected="selected" @endif>Non Asset</option>
													<option value="All" @if('All' == app('request')->input('asset_type')) selected="selected" @endif>All</option>											
												</select>												
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-3 pt-2">
											<button type="submit" class="btn btn-primary" name="search" value="search">Search</button>
											<a href="{{ route('admin.request.reports.inventory-valuation') }}" class="btn btn-warning">Reset</a>
										</div>
									</div>
								</form>
							</div>
