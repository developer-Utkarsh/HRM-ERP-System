<html>
	...
	<body>
		...
		<form action="{{ route('admin.request.po-request-edit') }}" method="post" enctype="multipart/form-data" class="po_request_new">
			@csrf
							...
							<div class="card-content border">
												...
												<td>
													...
													<select name="uom[]" class="w-100 uom1">
														...
														@foreach ($uom as $key => $val)
															<option value="{{ $val->code }}" {{ ($re->uom == $val->code) ? 'selected' : '' }}>
																{{ $val->code }}
															</option>
														@endforeach
													</select>
								...
								<div class="p-1">
									<div class="form-group">
										...
										<a href="{{ asset('laravel/public/po_upload') }}/<?=$record['quotation_one'];?>" target="_blank"><?=$record['quotation_one'];?></a>
									</div>
									<div class="form-group">
										<label>Quotation 2</label>
										<input type="file" name="quotation_two" value="" class="form-control blankvalue"/>
										
										<a href="{{ asset('laravel/public/po_upload') }}/<?=$record['quotation_two'];?>" target="_blank"><?=$record['quotation_two'];?></a>
									</div>
									<div class="form-group">
										<label>Quotation 3</label>
										<input type="file" name="quotation_three" value="" class="form-control blankvalue"/>
										
										<a href="{{ asset('laravel/public/po_upload') }}/<?=$record['quotation_three'];?>" target="_blank"><?=$record['quotation_three'];?></a>
									</div>
								</div>
			...
			<div class="text-center w-100 noPrint p-2">
				<div class="px-2 text-right">
					...
					<a href="{{ route('admin.request.poprint',[$id]) }}" class="btn btn-warning">Back</a>
				</div>							
			</div>
		</form>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.3/js/select2.min.js"></script>
		...
		$(document).ready(function() {
			...
			$.ajax({
				...
				data : {'_token' : '{{ csrf_token() }}', 'request_id': request_id,'is_edit':'1'},
				dataType : 'json',
				success : function (data){
					if(data.status == true){
