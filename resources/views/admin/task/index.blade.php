					...
					<table class="table data-list-view" style="background:#fff;">
						...
						<?php
							...
							foreach($taskDate as  $key => $task_val){  
									...
									<tr >
										<td><?=$i++;?></td>
										<td class="product-category">{{ $task_val->user ?  $task_val->user->name : '' }}</td>
										<td class="product-category">{{ date('d-m-Y',strtotime($task_val->date)) }}</td>
										...
										<td class="product-action">
											...
											{{--$task_details['task_date'] == date('Y-m-d')--}}
											@if($usr || Auth::user()->role_id == 29)
											<a href="{{ route('admin.task.edit', $task_val->id) }}">
												<span class="action-edit"><i class="feather icon-edit"></i></span>
											</a>
											<a href="{{ route('admin.task.task_delete', $task_val->id) }}" onclick="return confirm('Are You Sure To Delete Task')">
												<span class="action-delete"><i class="feather icon-trash"></i></span>
											</a>
											@endif 
											
											<a href="{{ route('admin.task.view', $task_val->id) }}">
