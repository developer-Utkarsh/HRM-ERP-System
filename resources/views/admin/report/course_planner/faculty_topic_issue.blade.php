			...
			<section id="data-list-view" class="data-list-view-header">
					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.course-planner.issue-raise-reports') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">
												<select class="form-control select-multiple3" name="assistant_id">
													...
													<option value="{{ $value->id }}" @if(app('request')->input('assistant_id') == $value->id) selected="selected" @endif>{{ $value->name }}</option>
													@endforeach
													@endif
												</select>
											</fieldset>
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">
												<!--<input type="text" name="remark" class="form-control" value="{{ app('request')->input('remark') }}">-->
												<select name="remark" class="select-multiple3 form-control">
													<option value="">Select</option>
													<option value="Yes" @if('Yes' == app('request')->input('remark')) selected="selected" @endif>Yes</option>
													<option value="No" @if('No' == app('request')->input('remark')) selected="selected" @endif>No</option>
												</select>
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-3">
											<button type="submit" class="btn btn-primary mt-1">Search</button>
											<a href="{{ route('admin.course-planner.issue-raise-reports') }}" class="btn btn-warning mt-1">Reset</a>
										</div>
									</div>
								</form>
							</div>
				...
				<div class="row mx-0">
					<div class="table-responsive">
						<table class="table data-list-view">
							...
							<tbody>	
								...
								<tr>
									...
									<td>{{ $topin_name }}</td>
									<td>{{ $val->topic_issue }}</td>
									<td>
										{{ $val->topic_issue_remark }}
										@if(empty($val->topic_issue_remark))
										  </br></br>
										 <button type="button" class="raise_issue" data-toggle="modal" data-target="#modalRaiseIssue" data-id="{{ $val->timetable_id }}">Remark</button>
										@endif
									</td>
								</tr>
								<?php $i++; } 
						...
						{!! $issue_report->appends($params)->links() !!}
						</div>
					</div>
				</form>                   
			</section>
