			...
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					...
					<div class="col-4 text-right">
						<a href="{{ route('admin.newtask.index') }}" class="btn btn-primary mr-1">Back</a>
					</div>
				</div>
			</div>
							...
							<div class="table-responsive">
								<table class="table data-list-view">
									...
									<tbody>
										...
										$assignBy = DB::table('users')->where('id', $value['task_added_by'])->first();
										$assignTo = DB::table('users')->where('id', $value['task_added_to'])->first(); 
										?>
										<tr>
											<td class="product-category">{{ $i++ }}</td>
											<td class="product-category">{{ isset($assignBy->name) ?  $assignBy->name : '' }}</td>
								            <td class="product-category">{{ isset($assignTo->name) ?  $assignTo->name : '' }}</td>
											<td class="product-category">{{ $value->task_title }}</td>
											...
											<td class="product-category">{{ $value->task_description }}</td>
											<td class="product-category">{{ $value->task_date }}</td>
										</tr>
										@endforeach							
									</tbody>
								</table>
