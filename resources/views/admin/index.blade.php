								...
								<div class="card-body pt-0">
									<div class="chart-info d-flex justify-content-between">
										<div class="text-center">
											...
											<a href="{{ url('admin/employees?status=Active') }}"><span class="font-large-1">{{$active}}</span></a>
										</div>
										@if(Auth::user()->role_id == 29 || Auth::user()->role_id == 24)
										<div class="text-center">
											<p class="mb-50">Inactive</p>
											<a href="{{ url('admin/employees?status=Inactive') }}"><span class="font-large-1">{{$inactive}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">Total</p>
											<a href="{{ route('admin.employees.index') }}"><span class="font-large-1">{{$total}}</span></a>
										</div>
										@endif
									</div>
								</div>
						...
						<div class="card">
							...
							<?php 
								if(!empty($comman_result)){
									foreach($comman_result as $comman_result_value){
							?>		
								<input type="hidden" name="totalPresent" value="{{ $comman_result_value->id }}" class="totalPresent"/>
							<?php
									}
								}								
							?>
							...
							?>		
								<input type="hidden" name="totalAbesent" value="{{ $value->id }}" class="totalAbesent"/>
							<?php
									}
								}								
							?>
								...
								<div class="card-body pt-0">
									<div class="chart-info d-flex justify-content-between">
										<div class="text-center">
											...
											<a href="{{ route('admin.attendance.fullattendence') }}"><span class="font-large-1">{{count($comman_result)}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">APP Present</p>
											<a href="{{ route('admin.attendance.index') }}"><span class="font-large-1">{{$app}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">RFID Present</p>
											<a href="{{ route('admin.attendance.rpattendance') }}"><span class="font-large-1">{{$rfid}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">Total Absent</p>
											<a href="{{ route('admin.attendance.absentfullattendence') }}">
											<span class="font-large-1">{{$active-count($comman_result)}}</span>
											<!--span class="font-large-1">{{$total_users-count($comman_result)}}</span-->
											</a>
										</div>
									</div>
								</div>
					...
					<div class="col-md-6 col-12">
						<div class="card">
							...
							@if(count($leave_data) > 0) 
								@php $total_leave = count($leave_data); @endphp
								@foreach($leave_data as $leave_data_value)
									@foreach($leave_data_value->leave_details as $leave_details_data_value)
										@if($leave_details_data_value->status == "Approved")
										@php $total_approved += 1; @endphp
										@elseif($leave_details_data_value->status == "Pending")
										@php $total_not_approved += 1; @endphp
										@endif
									@endforeach
								@endforeach
							@endif        
								...
								<div class="card-body pt-0">
									<div class="chart-info d-flex justify-content-between">
										<div class="text-center">
											...
											<a href="{{ url('admin/leave?status=Approved') }}"><span class="font-large-1">{{$total_approved}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">Today's Pending</p>
											<a href="{{ url('admin/leave?status=Pending') }}"><span class="font-large-1">{{$total_not_approved}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">Today's Total</p>
											<a href="{{ route('admin.leave.index') }}"><span class="font-large-1">{{$total_leave}}</span></a>
										</div>
									</div>
								</div>
							</div>
						...
						<div class="card">
							<div class="card-header d-flex flex-column align-items-start pb-0">
								...
								<div class="row" style="width: 100%;">
									...
									<div class="p-50 mt-1">
										@if($time_shift_count)
										<a href="{{ route('admin.employee.late-employee-list') }}" class="text-bold-700 mt-1 mb-25">{{$time_shift_count}}</a>
										@else
										<a href="" class="text-bold-700 mt-1 mb-25">{{$time_shift_count}}</a>
										@endif
									</div>
									</div>
								</div>
								...
								<div class="card-body pt-0">
									<div class="chart-info d-flex justify-content-between">
										<div class="text-center">
											...
											<a href="{{ route('admin.employees.birthday') }}"><span class="font-large-1">{{$b_day}}</span></a>
										</div>
										<div class="text-center">
											<p class="mb-50">Work Anniversary</p>
											<a href="{{ route('admin.employees.work-anniversary') }}"><span class="font-large-1">{{$work_anniversary}}</span></a>
										</div>
									</div>
								</div>
							</div>
