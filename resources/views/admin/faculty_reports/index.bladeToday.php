						...
						<body>
							...
							?>
								<tr>
									...
									<td>
										<?php
										if(strtotime(date('Y-m-d')) <= strtotime($get_faculty_timetable_value->cdate.'+1 day') || Auth::user()->user_details->degination == 'PRODUCTION INCHARGE' || Auth::user()->user_details->degination == 'CENTER HEAD'){
										?>
							...
							<tr>
								<td colspan="5"><b>Total Schedule Time:</b> 
								<?php
						...
						<div class="row pt-2">
							...
							<div class="col-md-12 col-12">
								<div class="form-label-group">
									<span for="first-name-column">Topic Name</span>
									<select class="form-control chapter_data select-multiple1" name="chapter_id" style= "width:100%;">
		...
		$.ajax({
			...
			success : function(data){
				if(data.status == false){
					...
					if(data.chapters != ''){
						$('.chapter_data').html(data.chapters);
					}
					if(data.res != ''){
						$('.assistant_id').html('');
