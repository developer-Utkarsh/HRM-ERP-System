					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.faculty-early-delay-reports') }}" method="get" name="filtersubmit">
									<div class="row">
									...
									<fieldset class="form-group" style="float:right;">		
										...
										<a href="{{ route('admin.faculty-early-delay-reports') }}" class="btn btn-warning">Reset</a>
							...
							?>
								<tr>
									...
									<td>
										...
										@if($get_faculty_timetable_value->delay_status==0 && $delay!='0')
										  <span class="delayWhatsapp btn btn-sm btn-danger" 
										  data-id="{{$get_faculty_timetable_value->id}}"
										  data-faculty_id="{{$get_faculty_timetable_value->faculty_id}}"
										  data-delay="{{$delay}}"
										  >
										  <i class="fa fa-check"></i> Approve</span>
										@elseif($get_faculty_timetable_value->delay_status==1)
										  Delay Approved
										@endif
									</td>
...
<script type="text/javascript">
	...
	$(".delayWhatsapp").on("click", function() {
		...
		$.ajax({
			...
			type: "POST",
			url : '{{ route('admin.delayWhatsapp') }}',
			data : {'_token' : '{{ csrf_token() }}', 'timetable_id': timetable_id,faculty_id:faculty_id,delay:delay},
			dataType : 'json',
