					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.holiday.index') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3" style="display:none;">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple type" name="type">
													@php $type = array('Public' => 'Public','Optional' => 'Optional'); @endphp
													<option value="">Select Any</option>
													@foreach($type as $key => $value)
													<option value="{{ $key }}" @if( app('request')->input('type') != '' && $key == app('request')->input('type')) selected="selected" @endif>{{ $value }}</option>
													@endforeach
												</select>												
											</fieldset>
										</div>
						...
						<tbody>
							@if(count($holiday) > 0)
								...
								<tr>
									...
									<td class="product-category"><span class="btn btn-sm btn-outline-{{$clr}}">{{ $sts }}</span></td>
									<td class="product-action">
										<a href="{{ route('admin.holiday.edit', $value->id) }}">
											<span class="action-edit"><i class="feather icon-edit"></i></span>
										</a>
										<a href="{{ route('admin.holiday.delete', $value->id) }}" onclick="return confirm('Are You Sure To Delete Holday')">
