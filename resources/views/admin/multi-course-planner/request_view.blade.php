	...
	<div class="content-wrapper">
		<div class="content-header row">
			...
			<div class="content-header-right text-md-right col-md-3 col-12 d-md-block">
				<a href="{{ route('admin.multi-course-planner.multi-planner-request') }}"><button class="btn btn-primary" type="button">Request Planner</button></a>
			</div>
			<?php } ?>
		</div>
							...
							<div class="card-content collapse show">
								<div class="card-body">
									<div class="users-list-filter">
										<form action="{{ route('admin.multi-course-planner.planner-request-view') }}" method="get" name="filtersubmit">
											<div class="row">
												...
												<div class="col-12 col-sm-6 col-lg-3">
													...
													<fieldset class="form-group">												
														<select class="form-control select-multiple status" name="status">
															@php $status = array('1' => 'Pending','2' => 'Approved','3' => 'Rejected'); @endphp
														...
														<select class="form-control select-multiple2 branch_location" name="branch_location">
															...
															@foreach($locations as $value)
																<option value="{{ $value->name }}"
																	@if(request()->input('branch_location') == $value->name || old('branch_location') == $value->name) selected @endif>
																	{{ ucwords($value->name) }}
																</option>
															@endforeach
														</select>												
												...
												<div class="col-12 col-sm-6 col-lg-3">
													...
													<a href="{{ route('admin.multi-course-planner.planner-request-view') }}" class="btn btn-warning">Reset</a>
													</fieldset>
												</div>
											</div>
										</form>
										...
										<table class="table data-list-view">
											...
											<tbody>
												...
												<tr>
													<td>{{ $i }}</td>													
													<td>{{ $re->cname }}</td>
													<td>{{ $re->planner_name }}</td>
													<td>{{ $re->city }}</td>
													<td>{{ $re->duration }}</td>
													<td>{{ $re->mode }}</td>
													<td>{{ date('d-m-Y',strtotime($re->timelines)) }}</td>
													<td>{{ $re->remark }}</td>
													<td>{{ $re->uname }}</td>
													<td>
														<?php															
													...
													<td>{{ $re->reason??'-' }}</td>
													<td>
														<button type="button" class="btn btn-primary btn-sm get_edit_data" data-id="{{ $re->id }}">View Status</button>
													</td>
													<td>
														...
														<a href="{{ route('admin.multi-course-planner.subject-assign', $re->id) }}" title="Subject & SME Assign">
														...
														<?php if($re->status==2){ ?>
														<a href="{{ route('admin.multi-course-planner.multi-planner-summary', $re->id) }}" title="Course Planner View">
															<i class="fa fa-eye"></i>
														</a>
														<?php } ?>
														
														<?php if($re->status==3 && Auth::user()->department_type==50){ ?>
														<a href="{{ route('admin.multi-course-planner.edit-multi-planner-request', $re->id) }}" title="Reopen Reject Request">
														...
														<i class="fa fa-upload uploadPlanner" aria-hidden="true" data-id="{{ $re->id }}"></i>
														<?php } ?>
													</td>
												</tr>
												<?php $i++; } 
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<form action="{{ route('admin.multi-course-planner.upload-planner') }}" method="post" enctype="multipart/form-data">
			@csrf
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="text-right"><a href="{{ asset('laravel/public/planner_upload.xlsx') }}" title="">Sample File</a></div>
				</div>
...
<script>
	...
	$(".get_edit_data").on("click", function() {  
		...
		$.ajax({
			type : 'POST',
			url : '{{ route('admin.multi-course-planner.show-work-status') }}',
