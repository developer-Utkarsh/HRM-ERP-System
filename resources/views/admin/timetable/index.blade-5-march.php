			...
			<section id="data-list-view" class="data-list-view-header">
							...
							<div class="users-list-filter">
								<form action="{{ route('admin.timetable.index') }}" method="get" name="filtersubmit">
									...
									<fieldset class="form-group" style="float:right;">		
										...
										<a href="{{ route('admin.timetable.index') }}" class="btn btn-warning">Reset</a>
										<a href="{{ route('admin.timetables.export') }}" class="btn btn-primary">Export</a>
									</fieldset>
								</form>
							</div>
						</div>
								...
								<table class="table table-responsive mb-0">
									<thead>
										<tr>											
											...
											<th scope="col">
												<div class="set_time" id="{{ $time->id }}"  style="position: unset !important;"><?=date('h:i A', strtotime($time->time_slot))?></div>
											</th>
											@endforeach
											@endif
										</tr>
				...
				<div id="myModal" class="modal fade">
					<div class="modal-dialog modal-xl">
						<div class="modal-content">
								...
								<div style="display:none;">
									...
									<div class="form-body">
										...
										<div class="row col-md-12 row_1">
											<div class="col-md-4 col-12 batch_online_faculty">
											...
											<div class="col-md-4 col-12 course_online_faculty" id="" style="display: none;">
											...
											<div class="col-md-4 col-12 subject_online_faculty" id="subject_loader">
												<div class="form-label-group">
											...
											<div class="col-md-4 col-12 faculty_online_faculty">
												<div class="form-label-group">
													<?php //$faculty = \App\User::where('role_id', '2')->where('status', '1')->orderBy('id', 'desc')->get(); ?>
										...
										<div class="addBatchesHtml" style="display:none">
											<div class="col-md-12 col-12 show_addbatch_icon">
												<a href='javascript:void(0);' title='Add' class='btn btn-primary btn-sm' onClick='addBatchMore()'><i class="ficon feather icon-plus"></i> Add More</a>
												<br/>
											</div>
											<br/>
											<div class="col-md-12 col-12 faculty_batches_div">
												
												
											</div>
										</div>
											
											<div class="row col-md-12 allAddMoreFields" style="display:">
												<div class="col-md-12 col-12 show_add_icon" style="display:;">
													<a href='javascript:void(0);' title='Add' class='add-more' onClick='addMore()' style='float: right;'><i class="ficon feather icon-plus"></i></a>
												</div>
												
												<div class="col-md-12 col-12 accrd_topic" style="display:;">
												<?php
												if (count($onlinebatch) > 0) {
													?>
													...
													<div class='form-label-group'>	
													<select class='form-control batch_accord_course' name='batch_accord_course[]'>
													...
													<div class='col-md-3 col-12'>	
													<div class='form-label-group'>	
													<select class='form-control batch_accord_subjects' name='batch_accord_subjects[]'>
													...
													<div class='col-md-3 col-12'>	
													<div class='form-label-group'>	
													<select class='form-control batch_accord_chapter' name='batch_accord_chapter[]'>
													...
													<div class='col-md-3 col-12'>	
													<div class='form-label-group'>	
													<select class='form-control batch_accord_topic' name='batch_accord_topic[]'>
							...
							<div class="row col-md-12 allFacultyBatchesHtml" style="display:none">
				...
				<script type="text/javascript">
					function selectRefresh() {
						...
						$('#submit_timetable_form .select-multiple-11').select2({
							width: '100%',
							placeholder: "Select Batch",
							allowClear: true
						});
						
						$('#submit_timetable_form .select-multiple-21').select2({
							width: '100%',
							placeholder: "Select Subject",
							allowClear: true
						});
						
						$('#submit_timetable_form .select-multiple-31').select2({
							width: '100%',
							placeholder: "Select Chapter",
							allowClear: true
						});
						
						$('#submit_timetable_form .select-multiple-41').select2({
							width: '100%',
				...
				$(document).on("change",".batch_accord_chapter", function () { 
						...
						if (chapter_id) {
							$.ajax({
								...
								type : 'POST',
								url : '{{ route('admin.get-topic-by-chapter') }}',
				...
				$(document).on("change", ".batch_accord_chapter", function () {
				  ...
				  <script type="text/javascript">
					$(document).on("change",".batch_id", function () {
						...
						var thisVal = $(this);
						var batch_id = thisVal.val();
						if (batch_id) {
							$.ajax({
								beforeSend: function(){
									thisVal.siblings("i").show();
								},
								type : 'POST',
								url : '{{ route('admin.get-course') }}',
								data : {'_token' : '{{ csrf_token() }}', 'batch_id': batch_id},
								dataType : 'html',
								success : function (data){
									var courser_js = thisVal.parents('.row_1').children('.course_online_faculty').children('div').children('.course_id');
									courser_js.empty();
									courser_js.append(data);
									courser_js.trigger("change");
									// $('.course_id').empty();
					...
					$(document).on("change",".course_id", function () { 
						...
						var batchThis = thisVal.parents('.row_1').children('.batch_online_faculty').children('div').children('.batch_id');
						var batch_id = batchThis.val();
						//get_subject(batch_id);
						if (course_id) {
							$.ajax({
								beforeSend: function(){
									batchThis.siblings("i").show();
								},
								type : 'POST',
								url : '{{ route('admin.get-class-batch-subject') }}',
								data : {'_token' : '{{ csrf_token() }}', 'batch_id': batch_id},
								dataType : 'html',
								success : function (data){
									batchThis.siblings("i").hide();
									var subject_js = thisVal.parents('.row_1').children('.subject_online_faculty').children('div').children('.subject_id');
									subject_js.empty();
									subject_js.append(data);
									// $('.subject_id').empty();
				...
				<script type="text/javascript">
					$(document).on("change",".subject_id", function () {
						...
						var thisVal = $(this);
						var subject_id = $(this).val();
						var batch_id = thisVal.parents('.row_1').children('.batch_online_faculty').children('div').children('.batch_id').val();
						// var batch_id = $(".batch_id").val();
						var course_id = $(".course_id").val();
						get_faculty(thisVal,subject_id,batch_id);
						get_remark(thisVal,subject_id);
						if (subject_id && course_id) {
							$.ajax({
								beforeSend: function(){
									thisVal.siblings("i").show();
								...
								dataType : 'html',
								success : function (data){
									thisVal.siblings("i").hide();
									thisVal.parents('.row_1').children('.show_remark').show();
									var chapter_js = thisVal.parents('.row_1').children('.select_chapter').children('div').children('.chapter_id');
									chapter_js.empty();
									chapter_js.append(data);
									// $('.chapter_id').empty();
									// $('.chapter_id').append(data);
								}
							});
							
							if($('input[name="class_type"]:checked').val()=="offline"){
								$.ajax({
									beforeSend: function(){
										thisVal.siblings("i").show();
									},
									type : 'POST',
									url : '{{ route('admin.get-chapter-and-topic') }}',
					...
					function get_faculty(thisVal,subject_id,batch_id){
						if (subject_id && batch_id) {
							var faculty_js = thisVal.parents('.row_1').children('.faculty_online_faculty').children('div');
							$.ajax({
								type : 'POST',
								...
								success : function (data){  console.log(JSON.parse(data));
									...
									faculty_js.children('.nt_av').text('');
									faculty_js.children('.faculty_id').empty();
									faculty_js.children('.faculty_id').append(resp.response);
									if(resp.today_leave){
										faculty_js.children('.nt_av').text('faculty today on leave');
										$('.dsabl').prop( "disabled", true );
									}
									
									if($('input[name="class_type"]:checked').val()=="online"){
										if(resp.batches_html != ''){
											$(".allFacultyBatchesHtml").html(resp.batches_html);
											$(".addBatchesHtml").show();
										}
										else{
										}
					...
					function get_remark(thisVal, subject_id){
						if (subject_id) {
							var remark_js = thisVal.parents('.row_1').children('.show_remark').children('div');
							$.ajax({
								type : 'POST',
								...
								success : function (data){
									// console.log(data);
									remark_js.children('.remark').val(data.data);
								}
							});
						}
					}
				...
				<script type="text/javascript">
					$(document).on("change",'.chapter_id', function () {
						...
						var chapter_id = $(this).val();
						var batch_id = thisVal.parents('.row_1').children('.batch_online_faculty').children('div').children('.batch_id').val();
						var course_id = thisVal.parents('.row_1').children('.course_online_faculty').children('div').children('.course_id').val();
						var subject_id = thisVal.parents('.row_1').children('.subject_online_faculty').children('div').children('.subject_id').val();
						if (chapter_id) {
							$.ajax({
								beforeSend: function(){
									thisVal.siblings("i").show();
								...
								success : function (data){
									thisVal.siblings("i").hide();
									var topic_js = thisVal.parents('.row_1').children('.select_topic').children('div').children('.topic_id');
									topic_js.empty();
									topic_js.append(data);
								}
							});
						}
					});
				...
				<script type="text/javascript">
					/* $(".topic_id").on("change", function () { 
							...
							$.ajax({
								type : 'POST',
								url : '{{ route('admin.get-data-by-topic') }}',
					...
					/* $(".subject_ids").on("change", function () { 
						...
						if (subject_id) {
							$.ajax({
								...
								type : 'POST',
								url : '{{ route('admin.get-batch-by-subject') }}',
				...
				$(".classess_type").on("click",function(){
					...
					if(type==1 && already !='online'){ //Online
						...
						$(".course_id,.subject_id,.faculty_id,.chapter_id,.topic_id,.chapter_id_offline").html('');
						$(".allFacultyBatchesHtml").html('');
					}
						...
						$(".course_id,.subject_id,.faculty_id,.chapter_id,.topic_id,.chapter_id_offline").html('');
						$(".allFacultyBatchesHtml").html('');
						$(".faculty_batches_div").html('');
						$(".addBatchesHtml").hide();
					}
					// alert($('input[name="class_type"]:checked').val());
				});
				...
				$(".delete_class").on("click", function() {
					...
					if(timetable_id){
						$.ajax({
							...
							type: "POST",
							url : '{{ route('admin.timetable.delete_class') }}',
				...
				$("#submit_timetable_online_form").submit(function(e) {
					...
					if(validatorprice.valid()){
						...
						$.ajax({
							...
							type: "POST",
							url : '{{ route('admin.timetable.edit-online-store') }}',
							data : dataForm,
							processData : false, 
							contentType : false,
							dataType : 'json',
						...
						$(".get_edit_data").on("click", function() { 
							...
							if(timetable_id){
								...
								$.ajax({
									...
									type : 'POST',
									url : '{{ route('admin.get-studio-edit') }}',
				...
				<script>
					function addBatchMore(){
						var html = $(".allFacultyBatchesHtml").html();
					    $(".faculty_batches_div").append(html);
						selectRefresh();
					...
					$(document).on("click",".batch_remove",function(){
						$(this).parents(".faculty_batches_div .row_1").remove();
					});
				</script>
