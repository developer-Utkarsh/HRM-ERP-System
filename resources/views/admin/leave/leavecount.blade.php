					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.leave.leavecount') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-6 col-sm-6 col-lg-3 pt-2" >
											...
											<a href="{{ route('admin.leave.leavecount') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
				...
				<div class="table-responsive">
					<table class="table data-list-view" style="background:#fff;">
						...
						<tbody >
							...
							<tr  >
								<td>{{ $uName }} ({{$register_id}})</td>
								<td>{{ !empty($pending_leaves->data->pending_pl)?$pending_leaves->data->pending_pl:0 }}</td>
								<td>{{ !empty($pending_leaves->data->pending_cl)?$pending_leaves->data->pending_cl:0 }}</td>
								<td>{{ !empty($pending_leaves->data->pending_comp_off)?$pending_leaves->data->pending_comp_off:0 }}</td>
							</tr>
							<?php $leave=[];  ?>
