			...
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					...
					<div class="col-4 text-right">
						<a href="{{ route('admin.leave.index') }}" class="btn btn-primary mr-1">Back</a>
					</div>
				</div>
			</div>
											...
											?>
												<div class="col-md-6 col-12">
													<div class="form-group">
														...
														<input type="date" class="form-control from_date" placeholder="Date" name="from_date" value="" min="{{$todayDate}}" required>
													</div>
												</div>
												<div class="col-md-6 col-12">
													<div class="form-group">
														<label for="first-name-column">To Date</label>
														<input type="date" class="form-control to_date" placeholder="Date" name="to_date" value="" min="{{$todayDate}}" required>
													</div>
												</div>
										...
										<div class="row fil remove_row">
												...
												<div class="col-md-3 col-12 date_div">
													<div class="form-group">
														...
														<input type="date" class="form-control leave_date" placeholder="Date" name="date[]" value="" @if(Auth::user()->id != 901)min="{{date('Y-m-d')}}"@endif required>
													</div>
												</div>
		...
		if(from_date && to_date){
			if (emp_id) {
				...
				$.ajax({
					...
					type : 'POST',
					url : '{{ route('admin.leave.append_leave_date') }}',
					data : {'_token' : '{{ csrf_token() }}', 'emp_id': emp_id,"from_date":from_date,"to_date":to_date},
					dataType : 'json',
					success : function (data){
						if(data.status == false){
							$(".append_div").html('');
