					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.leave.index') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple1 status" name="status">
													<option value="">Select Any</option>
													<option value="Pending" @if('Pending' == app('request')->input('status')) selected="selected" @endif>Pending</option>
													<option value="Approved" @if('Approved' == app('request')->input('status')) selected="selected" @endif>Approved</option>
													<option value="Rejected" @if('Rejected' == app('request')->input('status')) selected="selected" @endif>Rejected</option>
												</select>												
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-2">
									...
									<a href="{{ route('admin.leave.index') }}" class="btn btn-warning">Reset</a>
							...
							foreach($leave_array as  $key => $value){ 
								...
								if(!empty($value->user->id)){
									if(!empty($value->leave_details) && count($value->leave_details) > 0){
										...
										<tr >
											<td><?=$i++;?></td>
											<td class="product-category">{{ isset($value->user->register_id) ?  $value->user->register_id : '' }}</td>
											<td class="product-category">{{ isset($value->user->name) ?  $value->user->name : '' }}</td>
											<td class="product-category">{{ $branch_name }}</td>
											<td><?php echo isset($department->name)?$department->name:''; ?></td>
											<td>{{ $value->user_details->degination }}</td>
											<td class="product-category">{{ isset($leave_details->created_at) ?  date("d-m-Y", strtotime($leave_details->created_at)) : '' }}</td>
											<td class="product-category">{{ isset($leave_details->date) ?  date("d-m-Y", strtotime($leave_details->date)) : '' }}</td>
											...
											<td class="product-action">
												...
												if($leave_details->date >= $prev_month_date){
													...
													?>
														<a href="{{ route('admin.leave.edit', $leave_details->id.$nCheck) }}">
															<span class="action-edit"><i class="feather icon-edit"></i></span>
														</a>												
													<?php 
														...
														?>
															...
															<a href="{{ route('admin.leave.delete', $leave_details->id) }}">
												...
												?>
												<a href="{{ route('admin.leave.view', ['leave_id'=>$value->id,'leave_detail_id'=>$leave_details->id.$nCheck]) }}">
