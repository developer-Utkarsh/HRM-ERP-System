					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.faculty-hours-reports') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple2 faculty_id" name="faculty_id[]" multiple>
													...
													<option value="{{ $value->id }}"  <?php if(!empty( app('request')->input('faculty_id')) && is_array(app('request')->input('faculty_id')) > 0 && in_array($value->id, app('request')->input('faculty_id'))){ echo "selected"; } ?>>{{ $value->name }}</option>
													@endforeach
													@endif
												</select>												
											</fieldset>
									...
									<fieldset class="form-group" style="float:right;">		
										...
										<a href="{{ route('admin.faculty-hours-reports') }}" class="btn btn-warning">Reset</a>
										<?php if( Auth::user()->role_id != 3){ ?>
