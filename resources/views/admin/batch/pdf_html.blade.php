...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body id="printData">
		<div style="text-align:center;">
			<p>Batch Name : <span style="color:red">{{ $batch_name }}</span></p>
			<p>Course Name : <span style="color:red">{{ $course_name }}</span></p>
			<p>Start Date : <span style="color:red">{{ $start_date }}</span></p>
			<p>Subject Name : <span style="color:red">{{ $subject_name }}</span></p>
		</div>
		...
		<table class="table1" cellpadding="4" style='width:100%;' border="1">
			...
			<tbody>
				...
				<tr style="font-size:12px;" class="tHead">
					...
					<td>{{ $gt->topic_duration }} Minutes</td>
					<td>{{ $gt->tdate ? date('d-m-Y', strtotime($gt->tdate)) : '' }}</td>
				</tr>
				<?php $i++; } ?>
