									...
									<tbody>
										...
										<tr>
											...
											<td class="product-category">
												...
												<button type="button" id="download_pdf" data-id="{{ $value['subject_id'] }}" data-course-id="{{ $value['course_id'] }}" data-subject-name="{{ $value['subject_name'] }}" data-batch-name="{{ $batch->name }}" data-course-name="{{ $batch->course->name }}" data-start-date="{{ date('d-m-Y',strtotime($batch->start_date)) }}" data-batch-id="{{ $batch->id }}" class="btn-success btn-sm">PDF</button>
												<?php } ?>
...
$(".viewStatus").on("click",function(){
	...
	$.ajax({
		type: "GET",
		url : '{{ route('planerDetail') }}/0?batch_id='+batch_id+'&subject_id='+subject_id,
		dataType : 'json',
		success : function(data){
