					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.attendance-dashboard') }}" method="get" name="filtersubmit">
									<?php if(Auth::user()->role_id != 32){ ?>
									...
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple batch_id" name="batch_id[]" multiple>
													...
													<option value="{{ $value->batch_code }}" @if(in_array($value->batch_code,$batch_id)) selected="selected" @endif>{{ $value->name }}</option>
													@endforeach
												</select>												
											</fieldset>
										</div>										
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple anuprati_id" name="anuprati_id" >
													@php $anuprati = ['Anuprati Yojana','Anupriti Yojna-2022-23','Anupriti Yojna-2023-24','Anupriti Yojna-2023-24 (Ph-2)','Anuprati Yojna-2024-25']; @endphp
													<option value="">Select Any</option>
										...
										<div class="col-lg-8 text-right">
											...
											<fieldset class="form-group">		
												...
												<a href="{{ route('admin.attendance-dashboard') }}" class="btn btn-warning">Reset</a>
					...
					<table class="table data-list-view" id="my-table-id">
						...
						<tbody>
							...
							<tr>
								...
								<td class="sum_total_admission">{{ $value->total_admission }} </td>
								<td class="sum_total_anuprati">{{ $value->total_anuprati }}</td>
								<td class="sum_total_male">{{ $value->total_male }} </td>
								<td class="sum_total_female">{{ $total_female }} </td>
								<td class="sum_persentstudent"><a href="{{ route('admin.student-attendance-view',[$value->batch_id,$pdate,1]) }}" title="Click Here">{{$persentstudent}}</a></td>
								<td class="sum_absent"><a href="{{ route('admin.student-attendance-view',[$value->batch_id,$pdate,2]) }}" title="Click Here">{{$absent}}</a></td>
								<td>{{$pdate}}</td>
								<td>{{ $persentPercent }}</td>
							...
							<tr style="background: red;color: white;font-weight: bold;">
								<td><b>{{ app('request')->input('branch_location') }}</b></td>
								<td><b>Total</b></td>
...
<script type="text/javascript">
	...
	function locationBatch(value){
		...
		$.ajax({
			...
			data : {'_token' : '{{ csrf_token() }}', 'branch_id': value, 'fdate': fdate, 'tdate': tdate},
			dataType : 'json',
			success : function (data){
				if(data.status){
					$('.batch_id').empty();
	...
	$(document).ready(function() {
		...
		//	$("#overlay_loader").css('display','block');
			...
			$.ajax({
				...
				type: "POST",
				url : '{{ route('admin.batchinventory.studentSearch') }}',
