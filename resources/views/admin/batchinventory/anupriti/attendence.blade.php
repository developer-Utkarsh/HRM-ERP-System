					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ url('admin/anupriti-attendence') }}" method="get" name="filtersubmit">
									<input type="hidden" name="_token" value="{{ csrf_token() }}" />
									<div class="row">
										<div class="col-md-3 d-none">
											<label for="users-list-role">Application ID</label>
											<fieldset class="form-group">
												<input type="text" class="form-control name" id="se_name" name="application_id" placeholder="Application Id" value="@if(!empty(app('request')->input('application_id'))){{app('request')->input('application_id')}}@endif">
											</fieldset>
										</div>
										<input type="hidden" name="course" value="{{$_GET['course_2']??''}}">
										<div class="col-md-3">
										...
										<div class="col-md-3">
											...
											<fieldset class="form-group">
												<select class="form-control" name="location">
													...
													@if(($_GET['location']??'')=='Jodhpur') selected @endif>Jodhpur</option>
													<option value="Jaipur" 
													@if(($_GET['location']??'')=='Jaipur') selected @endif>Jaipur</option>
												</select>
											</fieldset>
										</div>
										<div class="col-md-3">
											...
											<fieldset class="form-group">
												<input type="month" class="form-control" name="month" value="@if(!empty(app('request')->input('month'))){{app('request')->input('month')}}@endif">
											</fieldset>
										</div>

										<div class="col-md-3">
											<input type="hidden" name="category" value="{{$_GET['category_2']??''}}">
											<label for="users-list-role">Category</label>
											<fieldset class="form-group">
												<select class="form-control" name="category_2">
													<option value="">Select</option>
													<option value="Minority" {{ request('category_2') == 'Minority' ? 'selected' : '' }}>Minority</option>
												</select>
				...
				<div id="content">
					...
					<div id="printData"> 
						<table style="font-size:12px;line-height:24px;border:solid 2px;width:100%;border-collapse:collapse;" class="table">
							...
							<tr>
								<td style="text-align:center" colspan="30"><b>Exam Name</b>: {{ $_GET['course_2']??'' }}, Month: {{ $_GET['month']??'' }}</td>
							</tr>
							<tr>
								<td colspan="38"></td>
							</tr>
...
<script type="text/javascript">		
	...
	$(document).ready(function () {
        var attendanceTable = $('#my-table-id').DataTable({
            ...
            "serverSide": true,
            "ajax":{
				"url": "{{ route('admin.std-anupriti-detail') }}",
				"dataType": "json",
				"type": "post",
				"data": function(data){
					console.log(data);
