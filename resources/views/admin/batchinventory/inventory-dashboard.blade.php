					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.inventory-dashboard') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('admin.inventory-dashboard') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
				...
				<div class="table-responsive">
					<table class="table data-list-view">
						...
						<tbody>
							...
							<tr>
								<td>{{ $i++ }}</td>
								<td>{{ $value->name }} - ({{ $value->inventory_type }})</td>
								<td>{{ $value->total_qty }}</td>
								<!--td>{{ $stock }}</td-->
								<td>{{ $total }}</td>
								<td><a href="{{ route('admin.student-inventory-view',[$value->batch_code,$value->id,1]) }}" title="Click Here">{{ $assign->given }}</a></td>
								<td><a href="{{ route('admin.student-inventory-view',[$value->batch_code,$value->id,2]) }}" title="Click Here">{{ $pending }}</a></td>
							</tr>
							<?php 		
									}
								}else{ 
