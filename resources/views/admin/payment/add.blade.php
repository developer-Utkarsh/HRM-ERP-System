					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('admin.payment.create') }}" method="get" id="npf_form">
									<div class="row">
...
<script type="text/javascript">		
	$(document).on('change', '.get_details', function () {
		...
		$.ajax({
			type : 'POST',
			url : '{{ route('admin.payment.npf-student-details') }}',
			data : {'_token' : '{{ csrf_token() }}', 'npfid': npfid,type:type},			
	...
	$(document).on('change', '.course_id', function () {
		...
		$.ajax({ 
			type : 'POST',
			url : '{{ route('admin.payment.npf-course-details') }}',
...
<script>
	...
	$("#verify_contact").submit(function(e) { 
		...
		$.ajax({
			...
			type: "POST",
			url : '{{ route('admin.payment.npf-sendpaymentlink') }}',
			data : dataForm,
			processData : false, 
			contentType : false,
			dataType : 'json',
