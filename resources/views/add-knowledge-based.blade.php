...
<html lang="en" >
	...
	<body style="padding:15px">
		<form action="{{ route('store-knowledge-based', $emp_id) }}" method="post">		
			@csrf	
			<div class="pt-3">
				<select class="form-control category_name" name="category_name">
					...
					@if(count($allCategory) > 0)
						@foreach($allCategory as $allCategoryValue)
							<option value="{{$allCategoryValue->id}}" @if($allCategoryValue->id == old('category_name')) selected="selected" @endif>{{$allCategoryValue->name}}</option>
						@endforeach
					@endif
				</select>
			...
			<div class="pt-3">
				<input type="text" name="reference_link" class="form-control reference_link" value="{{ old('reference_link') }}" placeholder="Reference Link"/>
