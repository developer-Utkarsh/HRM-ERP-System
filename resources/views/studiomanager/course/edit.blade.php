					...
					<div class="col-12">
						<div class="card">
							<div class="card-content">
								<div class="card-body">
									<form class="form" action="{{ route('studiomanager.course.update', $course->id) }}" method="post" enctype="multipart/form-data">
										@method('PATCH')
										@csrf
										<div class="form-body">
											<div class="row">
												<div class="col-md-6 col-12">
													<div class="form-group">
														...
														<input type="text" class="form-control" placeholder="Course Name" name="name" value="{{ old('name', $course->name) }}">
														@if($errors->has('name'))
												...
												<div class="col-md-6 col-12">
													<div class="form-group">
															...
															if(1){
																...
																<select class="form-control select-multiple" multiple="multiple" name="subject_id[]" style="width:100%;">
																	...
																	@foreach($subjects as $value)
																		<option value="{{ $value->id }}" @if(array_search($value->id, array_column($subject_course, 'subject_id')) !== false) selected="selected" @endif>{{ $value->name }}</option>
																		@endforeach
																</select>
																<?php
															}
														...
														<select class="form-control select-multiple" multiple="multiple" name="subject_id[]">
															<option value=""> - Select Subjects - </option>
															@foreach($subjects as $value)
															<option value="{{ $value->id }}" @foreach($course->course_subjects as $subject)
																@if($subject->pivot->subject_id == $value->id) selected="selected" @endif
													...
													<div class="col-md-6 col-12">
														<div class="form-group d-flex align-items-center">
															...
															<label>
																<input type="radio" name="status" value="1" {{ ($course->status == 1) ? "checked" : ""}}>
																Active
															</label>
															&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															<label>
																<input type="radio" name="status" value="0" {{ ($course->status == 0) ? "checked" : ""}}>
																Inactive
															</label>
														</div>
													</div>                                       
