			...
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					...
					<div class="col-4 text-right">
						<!--a href="{{ route('studiomanager.courses.import') }}" class="btn btn-primary mr-1">Import</a-->
					</div>
				</div>
			</div>
		</div>
					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('studiomanager.course.index') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('studiomanager.course.index') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
					...
					<table class="table data-list-view" id="">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									{{-- @if($value->status == "1")
									<a href="{{ route('studiomanager.course.status', $value->id) }}"><i class="fa fa-toggle-on"></i> Active </a>
									@else
									<a href="{{ route('studiomanager.course.status', $value->id) }}"><i class="fa fa-toggle-off"></i> Inactive </a>
									@endif --}}
									<a href="{{route('studiomanager.course.status', $value->id)}}">
								...
								<td class="product-action">
									<a href="{{ route('studiomanager.course.view', $value->id) }}">
										<span class="action-edit"><i class="feather icon-eye"></i></span>
									</a>
									<a href="{{ route('studiomanager.course.edit', $value->id) }}">
										<span class="action-edit"><i class="feather icon-edit"></i></span>
									</a>
									<a href="{{ route('studiomanager.course.delete', $value->id) }}" onclick="return confirm('Are You Sure To Delete Course')" class="d-none">
										<span class="action-delete"><i class="feather icon-trash"></i></span>
									</a>
								</td>
								<td>
									<a href="{{ route('studiomanager.course.export_csv', $value->id) }}" class="px-2">
										<span class="action-edit"><i class="feather icon-download"></i></span>
									</a>

									<a href="javascript:void(0);" data-toggle="modal" data-course-id="{{ $value->id }}" class="btn btn-sm btn-primary import_data"><span class="action-edit"><i class="feather icon-upload"></i></span></a>
								</td>
							</tr>
							@endforeach							
						</tbody>
	...
	$("#submit_import_file").submit(function(e) {
		...
		if(validatorprice.valid()){
			...
			$.ajax({
				...
				type: "POST",
				url : '{{ route('studiomanager.import-chapter') }}',
				data : dataForm,
				processData : false, 
				contentType : false,
				dataType : 'json',
