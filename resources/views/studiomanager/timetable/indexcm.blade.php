		...
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-left mb-0">Time Table CM</h2>
				...
				<div class="card-content collapse show">
					<div class="card-body">
						<div class="users-list-filter">
							<div class="row">
							<div class="col-md-7">
							<form action="{{ route('studiomanager.timetable.index') }}" method="get">
								<div class="row">
									<div class="col-md-6">
									...
									<div class="col-md-6">
										...
										<a href="{{ route('studiomanager.timetable.index') }}" class="btn btn-outline-warning">Reset</a>
							...
							<div class="col-md-12">
							@if(!empty(app('request')->input('tt_date')))
							<form action="{{ route('studiomanager.copy-timetable') }}" method="post" id="copy_form_tt">
								@csrf
								<div class="row">
									<div class="col-md-4">
										<label for="users-list-role">Date</label>
									...
									<div class="col-md-8">
										...
										@if(count($branch_locations) > 0 && in_array('barmer', $branch_locations))
							...
							@if(count($branch) > 0)
								...
								@foreach($branch as $branchval)
									<div class="">
										...
										?>	
										<div class="card-header mt-3" id="heading{{$i}}" style="padding: 0.5rem 0.5rem 0;">
											<h5 class="mb-0">
										...
										<div id="collapse{{$i}}" class="collapse card" aria-labelledby="heading{{$i}}" data-parent="#accordion">
											<div class="card-body main_div"> 
												...
												<div class="timetable-form hide">
													...
													<input type="hidden" name="sublocation"  class="sublocation" value="<?=$location;?>"/>
												...
												@if(count($check_timetable) > 0)
														...
														@php $j = 1; $multiple_batch_array = array(); $multiple_batch_str = ''; @endphp
														...
														<tr>
														<td>
														<form action='javascript:void(0)' method='get' class='edittimetable' id='editSubmitForm{{$j}}'>
														...
														<tr class='text-center add_row'>
															<td style='width:15%'>
																...
																<option value='Online Course Recording' @if($check_timetable_value->online_class_type == 'Online Course Recording'){{ "selected" }} @endif>Online Course Recording</option>
																
																<option value='YouTube Live' @if($check_timetable_value->online_class_type == 'YouTube Live'){{ "selected" }} @endif>YouTube Live</option>
																
																<option value='YouTube & App Live' @if($check_timetable_value->online_class_type == 'YouTube & App Live'){{ "selected" }} @endif>YouTube & App Live</option>
																
																<option value='Model Paper Recording' @if($check_timetable_value->online_class_type == 'Model Paper Recording'){{ "selected" }} @endif>Model Paper Recording</option>
																
																<option value='Offline' @if($check_timetable_value->online_class_type == 'Offline'){{ "selected" }} @endif>Offline</option>
																
																<option value='Offline & App live' @if($check_timetable_value->online_class_type == 'Offline & App live'){{ "selected" }} @endif>Offline & App live</option>
																
																<option value='App Live' @if($check_timetable_value->online_class_type == 'App Live'){{ "selected" }} @endif>App Live</option>
																
																<option value='Test' @if($check_timetable_value->online_class_type == 'Test'){{ "selected" }} @endif>Test</option>
																</select>																
																</fieldset>
															 </td>
															<td style='width:15%'>
																...
																@php $batch = \App\Batch::where('status', '1')->where('is_deleted', '0')->orderBy('id','desc')->get(); @endphp
																	...
																	<option value='{{$value->id}}' @if(count($multiple_batch_array) > 0 && in_array($value->id, $multiple_batch_array)){{'selected'}} @endif>{{$value->name}} ( {{$value->batch_code}} ) - {{$value->capacity}}</option>  
																	@endforeach
																	@endif
																	</select>												
																</fieldset>
															...
															<td style='width:15%'>
																<span class='edit_span s_studio_id'>{{$check_timetable_value->studios_name}} - {{$check_timetable_value->studios_capacity}}</span>
																<fieldset class='form-group hide'>
																@php $studio =  \App\Studio::with(['assistant'])->where('branch_id', $branchval->id)->where('status',1)->where('is_deleted','0')->get(); @endphp
																	...
																	<option value='{{$value->id}}' data-asst-id='{{$value->assistant_id}}'  @if(!empty($check_timetable_value->studio_id) && $check_timetable_value->studio_id == $value->id){{'selected'}} @endif>{{$value->name}} - {{$value->capacity}}</option>
																	@endif
																	@endforeach
																	@endif
																	</select>												
															...
															<td style='width:10%'>
																<span class='edit_span s_from_time'>{{date("h:i A", strtotime($check_timetable_value->from_time))}}</span>
																<fieldset class='form-group hide'>
															 ...
															 <td style='width:10%'>
																<span class='edit_span s_to_time'>{{date("h:i A", strtotime($check_timetable_value->to_time))}}</span>
																<fieldset class='form-group hide'>
															 ...
															 <td style='width:15%'>
																...
																<fieldset class='form-group hide'>
																	@php $faculty = \App\User::where('status', '1')->where('role_id', 2)->where('is_deleted','0')->orderBy('id','desc')->get(); @endphp
																	<select class='form-control select-multiple11 faculty' name='faculty_id[]' onChange='getSubjectByFaculty(this)'>
																	<option value=''> Select Faculty </option>
																	@if(count($faculty) > 0)
																	@foreach($faculty as $value)
																	<option value='{{$value->id}}'   @if(!empty($check_timetable_value->faculty_id) && $check_timetable_value->faculty_id == $value->id){{'selected'}} @endif>{{$value->name}}</option>
																	@endforeach
																	@endif 
																	</select>												
																</fieldset>
															...
															<td style='width:15%'>
																	...
																	<select class='form-control select-multiple11 subject_id' name='subject_id[]'>
																		@if(count($subjects) > 0)
																			@foreach($subjects as $subjects_val)
																				<option value='{{$subjects_val->id}}' @if(!empty($check_timetable_value->subject_id) && $check_timetable_value->subject_id == $subjects_val->id){{'selected'}}@endif>{{$subjects_val->name}}</option>
																			@endforeach	
																		@endif
																	</select>												
																</fieldset>
															 ...
															 <td style='width:10%'>
																<span class='edit_span s_new_remark'>{{ $check_timetable_value->remark}}</span>
																<fieldset class='form-group hide'>
																	<input type='text' name='new_remark[]' class='form-control new_remark' placeholder='Remark' value='{{$check_timetable_value->remark}}' autocomplete='off' style="width:120px">
																</fieldset>
															 </td>
															...
															<td style='width:5%'>
																<span class='edit_span'>
																	...
																	<a href='javascript:void(0)' class='float-right' data-id="{{$j}}" onclick="editTimetable(this)">
																		<span class='btn btn-success btn-sm action-edit edit_id'><i class='feather icon-edit'></i></span>
																	</a>
																	
																</span>
																<fieldset class='form-group hide'>
																	<button type='submit' id='time_table_edit_btn{{$j}}' data-id="{{$j}}" class='btn btn-outline-primary btn-sm float-right click_edit_class'>
															...
															<input type='hidden' class='cdate' name='cdate[]' value='{{$selecteddDate}}'>
															<input type='hidden' class='branch_id' name='branch_id[]' value='{{$branchval->id}}'>
														</div>
														</form>	
														</td></tr>

														@php $j++; $multiple_batch_array = array(); $multiple_batch_str = ''; @endphp
														@endforeach
														</table>			
														</div>
...
<div style="position:fixed;top:0;left:30%;z-index:999;font-size:14px;display:none;" id="subjectPop">
	<div class="card">
		<div class="card-body">
			<button type="button" class="btn btn-info" id="closeSubjectPop">Close</button>
...
function getBranchTimetable(branch_id,selecteddDate){
    if($('.branch_head'+branch_id).is(':empty')){
	    ...
	    $.ajax({
			type : 'POST',
			url : '{{ route('studiomanager.branch-wise-timetable') }}',
...
function confirmPublish(location){
	...
	if(copy_date_val == ''){
		...
		if(confirm('Are you sure to want publish '+location+' Timetable !')){
			$.ajax({
				type : 'POST',
				url : '{{ route('studiomanager.publish-timetable') }}',
...
function showDiv(e, branch_id){
	...
	if (branch_id) {
		$.ajax({
			type : 'POST',
			url : '{{ route('studiomanager.get-studio-by-branch-cm') }}',
...
function getSubjectByFaculty(e){ 
	...
	if (faculty_id) {
		$.ajax({
			type : 'POST',
			url : '{{ route('studiomanager.get-class-batch-subject-by-faculty') }}',
...
function getChapter(e){ 
	...
	$.ajax({
		...
		data : {'_token' : '{{ csrf_token() }}', 'course_id': course_id, 'subject_id': subject_id,},
		dataType : 'html',
		success : function (data){
...
function getTopic(e){ 
	...
	$.ajax({
		...
		data : {'_token' : '{{ csrf_token() }}', 'course_id': course_id,'batch_id':batch_id,'subject_id': subject_id,'chapter_id':chapter_id},
		dataType : 'html',
		success : function (data){
...
function deleteTimetable(e, id){
	if (id) {
		$.ajax({
			...
			type : 'POST',
			url : '{{ route('studiomanager.delete-timetable') }}',
		...
		submitHandler: function(forms) { 
				...
				// e.preventDefault();
					...
					$.ajax({
						...
						type: "POST",
						url : '{{ route('studiomanager.edit-timetable') }}',
						data : dataForm,
						processData : false, 
						contentType : false,
						dataType : 'json',
...
function fixedFaculty(e, branch_id){ 
	var checkClassType = $(e).val();
		...
		$.ajax({
			...
			type : 'POST',
			url : '{{ route('studiomanager.get-obs-studio') }}',
			data : {'_token' : '{{ csrf_token() }}','branch_id':branch_id,'is_obs':is_obs},
			dataType : 'html',
			success : function (data){
				$('.studio_id').empty();
...
function getSubjectFaculty(val,ress){
	var subject_id = val;
	var sublocation = $(ress).closest('form').find('.sublocation').val();
	...
	if(subject_id){
		...
		$.ajax({
			type : 'POST',
			url : '{{ route('studiomanager.get-subject-history') }}',
			data : {'_token' : '{{ csrf_token() }}', 'subject_id': subject_id,'sublocation':sublocation},
			dataType : 'json',
			success : function (res){
				$("#overlay_loader").css('display','none');
				$('#subjectPop').show();
				$('.subject_table').empty(); 
				$('.subject_table').append(res.data)
			}
		});
	}
}

$('#closeSubjectPop').on('click', function() {
    $('#subjectPop').hide();
});
</script>
@endsection
