			...
			<div class="content-header-left col-md-12 col-12 mb-2">
				<div class="row breadcrumbs-top">
					...
					<div class="col-4 text-right">
						<a href="{{ route('studiomanager.faculty-leave') }}" class="btn btn-primary mr-1">Back</a>
					</div>
				</div>
			</div>
												...
												<div class="col-md-6 col-12">
													<div class="form-group">
														...
														<select class="form-control emp_id select-multiple1" id="emp_id" name="emp_id" required>
															...
															<option value="{{ $value['id'] }}" @if($value['id'] == old('task_added_to')) selected="selected" @endif><?=$value['name']." (".$value['mobile'].")";?></option>
															@endforeach	
															@endif
														</select>
													</div>
...
<script type="text/javascript">
	...
	$("#leave_add").submit(function(e) {
		...
		$.ajax({
			...
			type: "POST",
			url : '{{ route('studiomanager.faculty_leave_add_save') }}',
			data : dataForm,
			processData : false,  
			contentType : false,
			dataType : 'json',
