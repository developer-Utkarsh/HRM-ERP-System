					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('studiomanager.batch.index') }}" method="get" name="filtersubmit">
									<div class="row">
											...
											<fieldset class="form-group">												
												<select class="form-control" name="branch">
													...
													@foreach($branch as $val)
													  <option value="{{ $val->branch }}" @if($val->branch == app('request')->input('branch')) selected="selected" @endif>{{ $val->branch }}</option>
													@endforeach
												</select>												
											</fieldset>
										</div>
										...
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple2" name="status">
													@php $status = ['Inactive', 'Active','Completed']; @endphp
													<option value="">Select Any</option>
										...
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple2" name="type">
													@php $type = ['online', 'offline','Live From Classroom']; @endphp
													<option value="">Select Any</option>
										...
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple2" name="cPlanner">
													<option value="">Select Any</option>
													<option value="1"  @if('1' == app('request')->input('cPlanner')) selected="selected" @endif>Yes</option>
													<option value="2" @if('2' == app('request')->input('cPlanner')) selected="selected" @endif>No</option>
												</select>												
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-2">
											...
											<fieldset class="form-group">
												<input type="text" class="form-control" name="bcode" placeholder="Batch Code" value="{{ app('request')->input('bcode') }}">
											</fieldset>
										</div>
										<div class="col-12 col-sm-6 col-lg-2">
											<label for="users-list-role">ERP ID</label>
											<fieldset class="form-group">
												<input type="text" class="form-control" name="erp_code" placeholder="ERP Code" value="{{ app('request')->input('erp_code') }}">
											</fieldset>
										</div>
									</div>
									<fieldset class="form-group" style="float:right;">		
									<button type="submit" class="btn btn-primary">Search</button>
									<a href="{{ route('studiomanager.batch.index') }}" class="btn btn-warning">Reset</a>
									</fieldset>
				...
				<div class="table-responsive">
					<table class="table data-list-view" id="">
						...
						<tbody>
							...
							<tr>
								<td>{{ $key + 1 }}</td>
								<td class="product-category">{{ $value->branch }}</td>
								...
								<td class="product-category" style="word-wrap:break-word;">{{ $value->batch_code }}</td>
								<td class="product-category">{{ isset($value->course->name) ? $value->course->name : '' }}</td>	
								<td class="product-category">{{ isset($value->category) ? $value->category : '' }}</td>	
								<td class="product-category">{{ $value->erp_course_id }}</td>	
								<td class="product-category">{{ isset($value->mentor->name) ? $value->mentor->name : '' }}</td>
								...
								<td class="product-category">@if($value->course_planer_enable == 1) Yes @else No @endif</td>
								<td class="product-category">@if($value->status == 1) Active @elseif($value->status == 2) Completed @else Inactive @endif</td>								
								<td class="product-action">
									<a href="{{ route('studiomanager.batch.edit', $value->id) }}">
										<span class="action-edit"><i class="feather icon-edit"></i></span>
									</a>
									<a href="{{ route('studiomanager.batch.delete', $value->id) }}" onclick="return confirm('Are You Sure To Delete Batch')" class="d-none">
										<span class="action-delete"><i class="feather icon-trash"></i></span>
									</a>
									<a href="{{ route('studiomanager.batch.view', $value->id) }}">
										<span class="action-edit"><i class="feather icon-eye"></i></span>
									</a>

									<a href="https://courses.utkarsh.com/offline-student-timetable/home.php?batch_name={{urlencode($value->name)}}&batch_code={{$value->batch_code}}" traget="_blank">
