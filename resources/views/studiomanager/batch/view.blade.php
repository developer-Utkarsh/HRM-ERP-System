									...
									<tbody>
										...
										<tr>
											...
											<td class="product-category">
												...
												<button type="button" id="download_pdf" data-id="{{ $value['subject_id'] }}" data-course-id="{{ $value['course_id'] }}" data-subject-name="{{ $value['subject_name'] }}" data-batch-name="{{ $batch->name }}" data-course-name="{{ $batch->course->name }}" data-start-date="{{ date('d-m-Y',strtotime($batch->start_date)) }}" class="btn-success btn-sm">PDF</button>
												<?php } ?>
...
$(".complete_submit").on("click",function(e) {
	...
	$.ajax({
		...
		type: "POST",
		url : '{{ route('studiomanager.batch_subject_status_update') }}',
