					...
					<div class="card-content collapse show">
						<div class="card-body" style="padding: 8px 0px 0px 18px;">
							<div class="users-list-filter">
								<form action="{{ route('studiomanager.studio-availability') }}" method="get" name="filtersubmit">
									<div class="row">

										<div class="col-12 col-md-3">
											<label for="users-list-status">Date</label>
											<fieldset class="form-group">												
												<input type="date" name="cdate" placeholder="Date" value="{{ app('request')->input('cdate') }}" class="form-control StartDateClass cdate">
											</fieldset>
										</div>
								...
								foreach($branchArray->studio as $value) { ?>
										...
										<tr>
											...
											if(count($value->timetable)>0){
												...
												foreach($value->timetable as $key => $timetable){ 
													...
													if($tt_count==$tt){
														...
														?>
															...
															<td colspan="<?=(int)$to_time-(int)$from_time+$add_1 ;?>" dd="6" tt="{{$to_time_dot}}">
																<?=$batch_name?>
																<?php if(!empty($batch_code)){
																	$curl = curl_init();
																	curl_setopt_array($curl, array(
												...
												if($to_time_dot < 19){
													?>
													<td colspan="<?=19-(int)$to_time+1 ;?>" dd="7" tt="{{$to_time_dot}}">  </td>
													<?php
												}
