					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('studiomanager.faculty-reports') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<input type="hidden" class="faculty_id_get" value="{{ json_encode(app('request')->input('faculty_id')) }}">
										...
										<div class="col-12 col-sm-6 col-lg-2">											
											...
											<fieldset class="form-group">																					
												<input type="date" name="fdate" value="{{ app('request')->input('fdate') }}" class="form-control StartDateClass fdate dateddmmyyy">	
											</fieldset>	
										</div>	
										...
										<div class="col-12 col-sm-6 col-lg-2">										
											...
											<fieldset class="form-group">										
												<input type="date" name="tdate" value="{{ app('request')->input('tdate') }}" class="form-control EndDateClass tdate dateddmmyyy">	
											</fieldset>									
										</div>										
										<div class="col-12 col-sm-6 col-lg-2">
										...
										<div class="col-12 col-md-3">
											...
											<fieldset class="form-group">												
												<select class="form-control select-multiple3 category_id" name="category_id">
													<option value="">Select Any</option>
													@if(!empty($get_category) && count($get_category) > 0)
													@foreach($get_category as $value)
													<option value="{{ $value->category }}" @if($value->category == app('request')->input('category_id')) selected="selected" @endif>{{ $value->category }}</option>
													@endforeach
													@endif
												</select>												
											</fieldset>
									...
									<fieldset class="form-group" style="float:right;">		
										...
										<a href="{{ route('studiomanager.faculty-reports') }}" class="btn btn-warning">Reset</a>
...
<script type="text/javascript">
	...
	$(".cancel-cls").on("click", function(){
		...
		if(remark != ''){
			 $.ajax({
				...
				type: "POST",
				url : '{{ route('studiomanager.update-cancel-class') }}',
			...
			success : function(data){
				if(data.status == false){
                    ...
                    for(var i=0;i<data['planner'].length;i++){
                    	...
                    	if(i==0){
                    	 batches+=`<div class="col-md-6 col-6"><label><input type="radio" name="batche_planner" class=" batche_planner" value="`+batch_id+`" checked="checked">&nbsp; `+batch_name+`</label></div>`;
                    	}else{
                    	  batches+=`<div class="col-md-6 col-6"><label><input type="radio" name="batche_planner" class=" batche_planner" value="`+batch_id+`">&nbsp; `+batch_name+`</label></div>`;	
                    	}
                    ...
                    for(var i=0;i<data['planner'].length;i++){
	                    ...
	                    chapter_topic_list+='<tr><td Colspan="3">'+batch_name+'</td></tr>';
	...
	$(".get_start_class_data_old").on("click", function() {
		...
		$.ajax({
			...
			type: "POST",
			url : '{{ route('studiomanager.edit-start-class') }}',
			data : {'_token' : '{{ csrf_token() }}', 'tt_id': tt_id},
			dataType : 'json',
			success : function(data){
				if(data.status == false){
	...
	$("#submit_start_class_form").submit(function(e) {
		...
		if(validatereschedule.valid()){
			$.ajax({
				...
				type: "POST",
				url : '{{ route('studiomanager.update-start-class') }}',
				data : dataForm,
				processData : false,  
				contentType : false,
				dataType : 'json',
	...
	$(document).on("change",".chapter_data", function () {
		...
		$.ajax({
			...
			data : {'_token' : '{{ csrf_token() }}', 'course_id': course_id,'subject_id': subject_id,'chapter_id':chapter_id,'batch_id':batch_id},
			dataType : 'html',
			success : function (data){
	...
	$(document).on("change",".topic_data", function () {
...
$(document).on('click', '#download_excel', function () {
    ...
    var allData = [];
    var headerRows = []; // store header row indices for styling

    $('.dataTable').each(function (index) {
        var ws_data = XLSX.utils.sheet_to_json(XLSX.utils.table_to_sheet(this), { header: 1 });

        // track where this header will appear in merged sheet
        var currentStartRow = allData.length;
        headerRows.push(currentStartRow);

        if (index > 0) allData.push([]); // add blank line between tables
    ...
    // 🎨 Apply background color to all header rows
    const range = XLSX.utils.decode_range(ws['!ref']);
    headerRows.forEach(rowIdx => {
        for (let C = range.s.c; C <= range.e.c; ++C) {
            const cell_ref = XLSX.utils.encode_cell({ r: rowIdx, c: C });
            if (!ws[cell_ref]) continue;
            ws[cell_ref].s = {
                fill: { patternType: "solid", fgColor: { rgb: "FFD700" } }, // gold background
                font: { bold: true }
            };
        }
    });
