						...
						<body>
							...
							?>
								<tr>
									...
									<?php 
									$get_batches_name = "";
									$get_batches = DB::table('timetables')->select("batch.name as b_name")->leftJoin('batch','batch.id','=','timetables.batch_id')->where('timetables.is_deleted', '0');
...
<script type="text/javascript">
	...
	$("body").on("click", "#download_pdf", function (e) {
		...
		/* window.location.href = "<?php echo URL::to('/studiomanager/'); ?>/faculty-report-pdf?" + Object.keys(data).map(function (k) {
			return encodeURIComponent(k) + '=' + encodeURIComponent(data[k])
		}).join('&'); */
	});
</script>
	...
	/* $(".branch_id").on("change", function () {
		...
		if (branch_id) {
			...
			$.ajax({
				...
				type : 'POST',
				url : '{{ route('studiomanager.get-branchwise-faculty') }}',
