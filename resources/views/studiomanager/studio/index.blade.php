					...
					<div class="card-content collapse show">
						<div class="card-body">
							<div class="users-list-filter">
								<form action="{{ route('studiomanager.studios.index') }}" method="get" name="filtersubmit">
									<div class="row">
										...
										<div class="col-12 col-sm-6 col-lg-3">
											...
											<a href="{{ route('studiomanager.studios.index') }}" class="btn btn-warning">Reset</a>
											</fieldset>
										</div>
									</div>
								</form>
					...
					<table class="table data-list-view">
						...
						<tbody>
							...
							<tr>
								...
								<td class="product-category">
									<a href="{{ route('studiomanager.studios.edit', $value->id) }}" class="btn btn-sm btn-primary">
										Change Assistant
									</a>
								...
								<td>
									{{-- @if($value->status == "1")
									<a href="{{ route('studiomanager.studios.status', $value->id) }}"><i class="fa fa-toggle-on"></i> Active </a>
									@else
									<a href="{{ route('studiomanager.studios.status', $value->id) }}"><i class="fa fa-toggle-off"></i> Inactive </a>
									@endif --}}
									<a href="{{route('studiomanager.studios.status', $value->id)}}">
										<strong class="fa fa-lg {{$value->status ? 'fa-toggle-on text-success' : 'fa-toggle-off text-grey'}}" title="Toggle publish"></strong>
									</a>
								</td>
								<td class="product-action">
									<a href="{{ route('studiomanager.studios.edit', $value->id) }}">
										<span class="action-edit"><i class="feather icon-edit"></i></span>
									</a>
									<a href="{{ route('studiomanager.studio.delete', $value->id) }}" onclick="return confirm('Are You Sure To Delete Studio')">
